var Ae=Object.defineProperty;var $e=(s,e,t)=>e in s?Ae(s,e,{enumerable:!0,configurable:!0,writable:!0,value:t}):s[e]=t;var W=(s,e,t)=>$e(s,typeof e!="symbol"?e+"":e,t);(function(){const e=document.createElement("link").relList;if(e&&e.supports&&e.supports("modulepreload"))return;for(const n of document.querySelectorAll('link[rel="modulepreload"]'))a(n);new MutationObserver(n=>{for(const i of n)if(i.type==="childList")for(const o of i.addedNodes)o.tagName==="LINK"&&o.rel==="modulepreload"&&a(o)}).observe(document,{childList:!0,subtree:!0});function t(n){const i={};return n.integrity&&(i.integrity=n.integrity),n.referrerPolicy&&(i.referrerPolicy=n.referrerPolicy),n.crossOrigin==="use-credentials"?i.credentials="include":n.crossOrigin==="anonymous"?i.credentials="omit":i.credentials="same-origin",i}function a(n){if(n.ep)return;n.ep=!0;const i=t(n);fetch(n.href,i)}})();const Ce="https://api.coingecko.com/api/v3/simple/price?ids=bitcoin,ethereum,ripple,kaspa&vs_currencies=eur,ars",Te="https://api.frankfurter.app/latest?from=EUR&to=USD,CHF,GBP,AUD";async function Le(){const s={EUR:1};try{const t=await(await fetch(Ce)).json();s.BTC=t.bitcoin.eur,s.ETH=t.ethereum.eur,s.XRP=t.ripple.eur,s.KAS=t.kaspa.eur,t.bitcoin.ars&&t.bitcoin.eur&&(s.ARS=t.bitcoin.eur/t.bitcoin.ars);const n=await(await fetch(Te)).json();s.USD=1/n.rates.USD,s.CHF=1/n.rates.CHF,s.GBP=1/n.rates.GBP,s.AUD=1/n.rates.AUD,s.GOLD=2100,s.SP500=4700}catch(e){console.error("Failed to fetch some prices:",e),s.USD=s.USD||.92,s.CHF=s.CHF||1.05,s.GBP=s.GBP||1.15,s.AUD=s.AUD||.6,s.ARS=s.ARS||.001}return s}class B{static async hash(e,t="salt_life_dashboard_2026"){const n=new TextEncoder().encode(e+t),i=await crypto.subtle.digest("SHA-512",n);return Array.from(new Uint8Array(i)).map(l=>l.toString(16).padStart(2,"0")).join("")}static async deriveVaultKey(e){return await this.hash(e,"vault_v4_dashboard_key")}static async deriveKey(e,t){const a=new TextEncoder,n=await crypto.subtle.importKey("raw",a.encode(e),{name:"PBKDF2"},!1,["deriveKey"]);return await crypto.subtle.deriveKey({name:"PBKDF2",salt:a.encode(t),iterations:25e4,hash:"SHA-512"},n,{name:"AES-GCM",length:256},!1,["encrypt","decrypt"])}static async encrypt(e,t){try{const a=crypto.getRandomValues(new Uint8Array(16)),n=crypto.getRandomValues(new Uint8Array(12)),i=await this.deriveKey(t,this.bufToBase64(a)),o=typeof e=="string"?e:JSON.stringify(e),l=new TextEncoder().encode(o),c=await crypto.subtle.encrypt({name:"AES-GCM",iv:n},i,l);return{payload:this.bufToBase64(new Uint8Array(c)),iv:this.bufToBase64(n),salt:this.bufToBase64(a),v:"5.0"}}catch(a){throw console.error("[Security] Encryption failed:",a),new Error("No se pudo encriptar la información")}}static async decrypt(e,t){try{if(!e||!e.payload||!e.iv||!e.salt)throw new Error("Formato de datos encriptados inválido");const{payload:a,iv:n,salt:i}=e,o=await this.deriveKey(t,i),l=await crypto.subtle.decrypt({name:"AES-GCM",iv:this.base64ToBuf(n)},o,this.base64ToBuf(a)),c=new TextDecoder().decode(l);try{return JSON.parse(c)}catch{return c}}catch(a){throw console.error("[Security] Decryption failed:",a),new Error("Contraseña incorrecta o datos corruptos")}}static bufToBase64(e){return btoa(String.fromCharCode(...new Uint8Array(e)))}static base64ToBuf(e){return new Uint8Array(atob(e).split("").map(t=>t.charCodeAt(0)))}}const ae=Object.freeze(Object.defineProperty({__proto__:null,SecurityService:B},Symbol.toStringTag,{value:"Module"})),x={MASTER_HASH:"db_master_hash",VAULT_KEY:"db_vault_key",BIO_ENABLED:"db_bio_enabled"};class k{static isSetup(){return!!localStorage.getItem(x.MASTER_HASH)}static async setup(e){const t=await B.hash(e),a=await B.deriveVaultKey(e);return localStorage.setItem(x.MASTER_HASH,t),sessionStorage.setItem(x.VAULT_KEY,a),a}static async unlock(e){const t=await B.hash(e),a=localStorage.getItem(x.MASTER_HASH);if(t===a){const n=await B.deriveVaultKey(e);return sessionStorage.setItem(x.VAULT_KEY,n),n}throw new Error("Contraseña incorrecta")}static async registerBiometrics(e){await this.unlock(e);const t=sessionStorage.getItem(x.VAULT_KEY);if(!window.PublicKeyCredential)throw new Error("Biometría no soportada en este dispositivo");try{const a=crypto.getRandomValues(new Uint8Array(32));return await navigator.credentials.create({publicKey:{challenge:a,rp:{name:"Life Dashboard",id:window.location.hostname},user:{id:crypto.getRandomValues(new Uint8Array(16)),name:"user",displayName:"User"},pubKeyCredParams:[{alg:-7,type:"public-key"}],timeout:6e4,authenticatorSelection:{authenticatorAttachment:"platform"},attestation:"none"}}),localStorage.setItem(x.BIO_ENABLED,"true"),localStorage.setItem(x.VAULT_KEY,t),!0}catch(a){throw console.error("Biometric setup failed:",a),new Error("Error al configurar biometría")}}static async unlockWithBiometrics(){if(!(localStorage.getItem(x.BIO_ENABLED)==="true"))throw new Error("Biometría no activada");try{const t=crypto.getRandomValues(new Uint8Array(32));await navigator.credentials.get({publicKey:{challenge:t,rpId:window.location.hostname,userVerification:"required",timeout:6e4}});const a=localStorage.getItem(x.VAULT_KEY);if(a)return sessionStorage.setItem(x.VAULT_KEY,a),a;throw new Error("Llave no encontrada. Usa contraseña.")}catch(t){throw console.error("Biometric auth failed:",t),new Error("Fallo de identificación biométrica")}}static logout(){sessionStorage.removeItem(x.VAULT_KEY)}static getVaultKey(){return sessionStorage.getItem(x.VAULT_KEY)}static isBioEnabled(){return localStorage.getItem(x.BIO_ENABLED)==="true"}}const Be="974464877836-721dprai6taijtuufmrkh438q68e97sp.apps.googleusercontent.com",Pe="https://www.googleapis.com/auth/drive.file";class ${static hasToken(){return!!this.accessToken}static async init(){return new Promise((e,t)=>{const a=()=>{window.gapi&&window.google?gapi.load("client",async()=>{try{await gapi.client.init({discoveryDocs:["https://www.googleapis.com/discovery/v1/apis/drive/v3/rest"]}),this.tokenClient=google.accounts.oauth2.initTokenClient({client_id:Be,scope:Pe,callback:n=>{if(n.error){console.error("[Drive] Auth callback error:",n);return}this.accessToken=n.access_token,gapi.client.setToken({access_token:n.access_token}),sessionStorage.setItem("drive_access_token",n.access_token)}}),this.accessToken&&gapi.client.setToken({access_token:this.accessToken}),e(!0)}catch(n){console.error("[Drive] Init error:",n),t(n)}}):setTimeout(a,150)};a()})}static async authenticate(){return this.tokenClient||await this.init(),new Promise((e,t)=>{this.tokenClient.callback=a=>{if(a.error){t(new Error(a.error_description||"Fallo en la autenticación"));return}this.accessToken=a.access_token,gapi.client.setToken({access_token:a.access_token}),sessionStorage.setItem("drive_access_token",a.access_token),e(a.access_token)},this.tokenClient.requestAccessToken({prompt:"consent"})})}static async getOrCreateFolderPath(e){var n;(n=gapi.client)!=null&&n.drive||await this.init();const t=e.split("/").filter(i=>i);let a="root";for(const i of t)try{const o=`name = '${i}' and mimeType = 'application/vnd.google-apps.folder' and '${a}' in parents and trashed = false`,c=(await gapi.client.drive.files.list({q:o,fields:"files(id, name)"})).result.files;if(c&&c.length>0)a=c[0].id;else{const p={name:i,mimeType:"application/vnd.google-apps.folder",parents:[a]};a=(await gapi.client.drive.files.create({resource:p,fields:"id"})).result.id}}catch(o){throw o.status===401?(this.accessToken=null,new Error("Sesión de Google expirada. Por favor reconecta.")):o}return a}static async pushData(e,t){var a,n,i;try{if(!this.accessToken)throw new Error("Cloud not connected");(a=gapi.client)!=null&&a.drive||await this.init(),gapi.client.setToken({access_token:this.accessToken}),console.log("[Drive] Pushing full encrypted data...");const o=await this.getOrCreateFolderPath("/backup/life-dashboard/"),l=await B.encrypt(e,t),c="dashboard_vault_v5.bin",p=`name = '${c}' and '${o}' in parents and trashed = false`,d=(await gapi.client.drive.files.list({q:p,fields:"files(id)"})).result.files,v=new Blob([JSON.stringify(l)],{type:"application/json"});if(d&&d.length>0){const h=d[0].id,f=await fetch(`https://www.googleapis.com/upload/drive/v3/files/${h}?uploadType=media`,{method:"PATCH",headers:{Authorization:`Bearer ${this.accessToken}`},body:v});if(!f.ok)throw new Error(`Error al actualizar backup: ${f.status}`)}else{const h={name:c,parents:[o]},f=new FormData;f.append("metadata",new Blob([JSON.stringify(h)],{type:"application/json"})),f.append("file",v);const E=await fetch("https://www.googleapis.com/upload/drive/v3/files?uploadType=multipart",{method:"POST",headers:{Authorization:`Bearer ${this.accessToken}`},body:f});if(!E.ok)throw new Error(`Error al crear backup: ${E.status}`)}return!0}catch(o){throw console.error("[Drive] Push failed:",o),new Error(((i=(n=o.result)==null?void 0:n.error)==null?void 0:i.message)||o.message||"Fallo al subir datos a Drive")}}static async pullData(e){var t,a,n;try{if(!this.accessToken)throw new Error("Cloud not connected");(t=gapi.client)!=null&&t.drive||await this.init(),gapi.client.setToken({access_token:this.accessToken}),console.log("[Drive] Pulling data...");const l=`name = 'dashboard_vault_v5.bin' and '${await this.getOrCreateFolderPath("/backup/life-dashboard/")}' in parents and trashed = false`,p=(await gapi.client.drive.files.list({q:l,fields:"files(id, name)"})).result.files;if(!p||p.length===0)return null;const r=p[0].id,d=await fetch(`https://www.googleapis.com/drive/v3/files/${r}?alt=media`,{headers:{Authorization:`Bearer ${this.accessToken}`}});if(!d.ok)throw new Error(`Error al descargar backup: ${d.status}`);const v=await d.json();return await B.decrypt(v,e)}catch(i){throw console.error("[Drive] Pull failed:",i),new Error(((n=(a=i.result)==null?void 0:a.error)==null?void 0:n.message)||i.message||"Fallo al recuperar datos de Drive")}}static async deleteBackup(){var e,t,a;try{if(!this.accessToken)throw new Error("Cloud not connected");(e=gapi.client)!=null&&e.drive||await this.init(),gapi.client.setToken({access_token:this.accessToken});const o=`name = 'dashboard_vault_v5.bin' and '${await this.getOrCreateFolderPath("/backup/life-dashboard/")}' in parents and trashed = false`,c=(await gapi.client.drive.files.list({q:o,fields:"files(id)"})).result.files;if(c&&c.length>0){const p=c[0].id;return await gapi.client.drive.files.delete({fileId:p}),console.log("[Drive] Backup deleted successfully"),!0}return!1}catch(n){throw console.error("[Drive] Deletion failed:",n),new Error(((a=(t=n.result)==null?void 0:t.error)==null?void 0:a.message)||n.message||"Fallo al borrar backup en Drive")}}}W($,"tokenClient",null),W($,"accessToken",sessionStorage.getItem("drive_access_token")||null);const se={wallet:'<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M21 12V7H5a2 2 0 0 1 0-4h14v4"/><path d="M3 5v14a2 2 0 0 0 2 2h16v-5"/><path d="M18 12a2 2 0 0 0 0 4h4v-4Z"/></svg>',target:'<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><circle cx="12" cy="12" r="10"/><circle cx="12" cy="12" r="6"/><circle cx="12" cy="12" r="2"/></svg>',calendar:'<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><rect width="18" height="18" x="3" y="4" rx="2" ry="2"/><line x1="16" x2="16" y1="2" y2="6"/><line x1="8" x2="8" y1="2" y2="6"/><line x1="3" x2="21" y1="10" y2="10"/></svg>',heart:'<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M19 14c1.49-1.46 3-3.21 3-5.5A5.5 5.5 0 0 0 16.5 3c-1.76 0-3 .5-4.5 2-1.5-1.5-2.74-2-4.5-2A5.5 5.5 0 0 0 2 8.5c0 2.3 1.5 4.05 3 5.5l7 7Z"/></svg>',settings:'<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M12.22 2h-.44a2 2 0 0 0-2 2v.18a2 2 0 0 1-1 1.73l-.43.25a2 2 0 0 1-2 0l-.15-.08a2 2 0 0 0-2.73.73l-.22.38a2 2 0 0 0 .73 2.73l.15.1a2 2 0 0 1 1 1.72v.51a2 2 0 0 1-1 1.74l-.15.09a2 2 0 0 0-.73 2.73l.22.38a2 2 0 0 0 2.73.73l.15-.08a2 2 0 0 1 2 0l.43.25a2 2 0 0 1 1 1.73V20a2 2 0 0 0 2 2h.44a2 2 0 0 0 2-2v-.18a2 2 0 0 1 1-1.73l.43-.25a2 2 0 0 1 2 0l.15.08a2 2 0 0 0 2.73-.73l.22-.39a2 2 0 0 0-.73-2.73l-.15-.08a2 2 0 0 1-1-1.74v-.5a2 2 0 0 1 1-1.74l.15-.09a2 2 0 0 0 .73-2.73l-.22-.38a2 2 0 0 0-2.73-.73l-.15.08a2 2 0 0 1-2 0l-.43-.25a2 2 0 0 1-1-1.73V4a2 2 0 0 0-2-2z"/><circle cx="12" cy="12" r="3"/></svg>',building:'<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><rect width="16" height="20" x="4" y="2" rx="2" ry="2"/><path d="M9 22v-4h6v4"/><path d="M8 6h.01"/><path d="M16 6h.01"/><path d="M12 6h.01"/><path d="M12 10h.01"/><path d="M12 14h.01"/><path d="M16 10h.01"/><path d="M16 14h.01"/><path d="M8 10h.01"/><path d="M8 14h.01"/></svg>',home:'<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="m3 9 9-7 9 7v11a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2z"/><polyline points="9 22 9 12 15 12 15 22"/></svg>',trendingUp:'<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><polyline points="22 7 13.5 15.5 8.5 10.5 2 17"/><polyline points="16 7 22 7 22 13"/></svg>',bitcoin:'<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M11.767 19.089c4.924.868 6.14-6.025 1.216-6.894m-1.216 6.894L5.86 18.047m5.908 1.042-.347 1.97m1.563-8.864c4.924.869 6.14-6.025 1.215-6.893m-1.215 6.893-3.94-.694m5.155-6.2L8.29 4.26m5.908 1.042.348-1.97M7.48 20.364l3.126-17.727"/></svg>',dollarSign:'<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><line x1="12" x2="12" y1="2" y2="22"/><path d="M17 5H9.5a3.5 3.5 0 0 0 0 7h5a3.5 3.5 0 0 1 0 7H6"/></svg>',car:'<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M19 17h2c.6 0 1-.4 1-1v-3c0-.9-.7-1.7-1.5-1.9C18.7 10.6 16 10 16 10s-1.3-1.4-2.2-2.3c-.5-.4-1.1-.7-1.8-.7H5c-.6 0-1.1.4-1.4.9l-1.5 2.8C1.4 11.3 1 12.1 1 13v3c0 .6.4 1 1 1h2"/><circle cx="7" cy="17" r="2"/><path d="M9 17h6"/><circle cx="17" cy="17" r="2"/></svg>',creditCard:'<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><rect width="20" height="14" x="2" y="5" rx="2"/><line x1="2" x2="22" y1="10" y2="10"/></svg>',landmark:'<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><line x1="3" x2="21" y1="22" y2="22"/><line x1="6" x2="6" y1="18" y2="11"/><line x1="10" x2="10" y1="18" y2="11"/><line x1="14" x2="14" y1="18" y2="11"/><line x1="18" x2="18" y1="18" y2="11"/><polygon points="12 2 20 7 4 7"/></svg>',plus:'<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M5 12h14"/><path d="M12 5v14"/></svg>',x:'<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M18 6 6 18"/><path d="m6 6 12 12"/></svg>',trash:'<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M3 6h18"/><path d="M19 6v14c0 1-1 2-2 2H7c-1 0-2-1-2-2V6"/><path d="M8 6V4c0-1 1-2 2-2h4c1 0 2 1 2 2v2"/></svg>',edit:'<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M17 3a2.85 2.83 0 1 1 4 4L7.5 20.5 2 22l1.5-5.5Z"/><path d="m15 5 4 4"/></svg>',chevronRight:'<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="m9 18 6-6-6-6"/></svg>',chevronLeft:'<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="m15 18-6-6 6-6"/></svg>',calculator:'<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><rect width="16" height="20" x="4" y="2" rx="2"/><line x1="8" x2="16" y1="6" y2="6"/><line x1="16" x2="16" y1="14" y2="18"/><path d="M16 10h.01"/><path d="M12 10h.01"/><path d="M8 10h.01"/><path d="M12 14h.01"/><path d="M8 14h.01"/><path d="M12 18h.01"/><path d="M8 18h.01"/></svg>',arrowUpRight:'<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M7 7h10v10"/><path d="M7 17 17 7"/></svg>',arrowDownRight:'<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="m7 7 10 10"/><path d="M17 7v10H7"/></svg>',piggyBank:'<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M19 5c-1.5 0-2.8 1.4-3 2-3.5-1.5-11-.3-11 5 0 1.8 0 3 2 4.5V20h4v-2h3v2h4v-4c1-.5 1.7-1 2-2h2v-4h-2c0-1-.5-1.5-1-2V5z"/><path d="M2 9v1c0 1.1.9 2 2 2h1"/><path d="M16 11h.01"/></svg>',receipt:'<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M4 2v20l2-1 2 1 2-1 2 1 2-1 2 1 2-1 2 1V2l-2 1-2-1-2 1-2-1-2 1-2-1-2 1Z"/><path d="M16 8h-6a2 2 0 1 0 0 4h4a2 2 0 1 1 0 4H8"/><path d="M12 17.5v-11"/></svg>',coins:'<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><circle cx="8" cy="8" r="6"/><path d="M18.09 10.37A6 6 0 1 1 10.34 18"/><path d="M7 6h1v4"/><path d="m16.71 13.88.7.71-2.82 2.82"/></svg>',scale:'<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="m16 16 3-8 3 8c-.87.65-1.92 1-3 1s-2.13-.35-3-1Z"/><path d="m2 16 3-8 3 8c-.87.65-1.92 1-3 1s-2.13-.35-3-1Z"/><path d="M7 21h10"/><path d="M12 3v18"/><path d="M3 7h2c2 0 5-1 7-2 2 1 5 2 7 2h2"/></svg>',briefcase:'<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><rect width="20" height="14" x="2" y="7" rx="2" ry="2"/><path d="M16 21V5a2 2 0 0 0-2-2h-4a2 2 0 0 0-2 2v16"/></svg>',zap:'<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><polygon points="13 2 3 14 12 14 11 22 21 10 12 10 13 2"/></svg>',download:'<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M21 15v4a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2v-4"/><polyline points="7 10 12 15 17 10"/><line x1="12" x2="12" y1="15" y2="3"/></svg>',downloadCloud:'<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M4 14.899A7 7 0 1 1 15.71 8h1.79a4.5 4.5 0 0 1 2.5 8.242"/><path d="M12 12v9"/><path d="m8 17 4 4 4-4"/></svg>',cloud:'<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M17.5 19H9a7 7 0 1 1 6.71-9h1.79a4.5 4.5 0 1 1 0 9Z"/></svg>',shield:'<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M12 22s8-4 8-10V5l-8-3-8 3v7c0 6 8 10 8 10z"/></svg>',link:'<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M10 13a5 5 0 0 0 7.54.54l3-3a5 5 0 0 0-7.07-7.07l-1.72 1.71"/><path d="M14 11a5 5 0 0 0-7.54-.54l-3 3a5 5 0 0 0 7.07 7.07l1.71-1.71"/></svg>',refreshCw:'<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M3 12a9 9 0 0 1 9-9 9.75 9.75 0 0 1 6.74 2.74L21 8"/><path d="M21 3v5h-5"/><path d="M21 12a9 9 0 0 1-9 9 9.75 9.75 0 0 1-6.74-2.74L3 16"/><path d="M8 16H3v5"/></svg>',lock:'<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><rect width="18" height="11" x="3" y="11" rx="2" ry="2"/><path d="M7 11V7a5 5 0 0 1 10 0v4"/></svg>',logOut:'<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M9 21H5a2 2 0 0 1-2-2V5a2 2 0 0 1 2-2h4"/><polyline points="16 17 21 12 16 7"/><line x1="21" x2="9" y1="12" y2="12"/></svg>',package:'<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="m7.5 4.27 9 5.15"/><path d="M21 8a2 2 0 0 0-1-1.73l-7-4a2 2 0 0 0-2 0l-7 4A2 2 0 0 0 3 8v8a2 2 0 0 0 1 1.73l7 4a2 2 0 0 0 2 0l7-4A2 2 0 0 0 21 16Z"/><path d="m3.3 7 8.7 5 8.7-5"/><path d="M12 22V12"/></svg>'};function m(s,e=""){return(se[s]||se.package).replace("<svg",`<svg class="${e}"`)}class Re{constructor(){this.toastContainer=null,this._initToastContainer()}_initToastContainer(){document.getElementById("toast-container")||(this.toastContainer=document.createElement("div"),this.toastContainer.id="toast-container",this.toastContainer.className="toast-container",document.body.appendChild(this.toastContainer))}toast(e,t="success",a=3e3){const n=document.createElement("div");n.className=`toast toast-${t} stagger-in`;const i=t==="success"?"check":t==="error"?"alertCircle":"info";n.innerHTML=`
  ...
  `}function Fe(s){const e=document.getElementById("edit-modal"),t=document.getElementById("edit-modal-close"),a=document.getElementById("btn-update"),n=document.getElementById("btn-delete");e.addEventListener("click",i=>{i.target===e&&q()}),t.addEventListener("click",q),a.addEventListener("click",()=>je(s)),n.addEventListener("click",()=>Ge(s))}function je(s){var c,p,r,d,v,h,f,E;const e=(p=(c=document.getElementById("edit-name"))==null?void 0:c.value)==null?void 0:p.trim(),t=(r=document.getElementById("edit-currency"))==null?void 0:r.value,a=(v=(d=document.getElementById("edit-details"))==null?void 0:d.value)==null?void 0:v.trim(),n=parseFloat((h=document.getElementById("edit-value"))==null?void 0:h.value)||0,i=parseFloat((f=document.getElementById("edit-amount"))==null?void 0:f.value)||0,o=parseFloat((E=document.getElementById("edit-monthly"))==null?void 0:E.value)||0;if(!e){g.alert("Requerido","El nombre es obligatorio para guardar los cambios.");return}const l={name:e,currency:t,details:a};s.fields.includes("value")&&(l.value=n),s.fields.includes("amount")&&(l.amount=i),s.fields.includes("monthlyIncome")&&(l.monthlyIncome=o),s.fields.includes("monthlyPayment")&&(l.monthlyPayment=o),u[s.updateMethod](V.id,l),q()}function Ge(s){g.confirm("¿Eliminar?",`¿Estás seguro de que quieres borrar "${V.name}"? Esta acción no se puede deshacer.`).then(e=>{e&&(u[s.deleteMethod](V.id),g.toast("Eliminado correctamente","info"),q())})}function q(){const s=document.getElementById("edit-modal");s&&(s.classList.remove("active"),setTimeout(()=>s.remove(),300)),V=null,H=null}function re(){const s=u.getState(),e=s.currencySymbol,t=u.getPassiveIncome(),a=u.getLivingExpenses(),n=u.getNetPassiveIncome(),i=u.getInvestmentAssetsValue(),o=u.getTotalLiabilities(),l=u.getNetWorth(),c=u.getAllIncomes(),p=u.getAllExpenses(),r=u.getNetIncome();return`
  ...
  `}function He(s){const e=[...s.passiveAssets,...s.investmentAssets],t=s.liabilities;if(e.length===0)return"";const a={Bitcoin:{value:0,color:"#f59e0b"},Altcoins:{value:0,color:"#6366f1"},Inmuebles:{value:0,color:"#a855f7"},Bolsa:{value:0,color:"#00d4aa"},Oro:{value:0,color:"#fbbf24"},"Otros/Efe.":{value:0,color:"#94a3b8"}};e.forEach(r=>{const d=u.convertValue(r.value||0,r.currency||"EUR");r.currency==="BTC"?a.Bitcoin.value+=d:r.currency==="ETH"||r.currency==="XRP"||r.type==="crypto"?a.Altcoins.value+=d:r.type==="property"||r.type==="rental"?a.Inmuebles.value+=d:r.type==="stocks"||r.type==="etf"||r.currency==="SP500"?a.Bolsa.value+=d:r.currency==="GOLD"?a.Oro.value+=d:a["Otros/Efe."].value+=d});const n=t.filter(r=>r.type==="mortgage").reduce((r,d)=>r+u.convertValue(d.amount||0,d.currency||"EUR"),0);a.Inmuebles.value=Math.max(0,a.Inmuebles.value-n),s.hideRealEstate&&(a.Inmuebles.value=0);const i=Object.entries(a).filter(([r,d])=>d.value>0).sort((r,d)=>d[1].value-r[1].value),o=i.reduce((r,[d,v])=>r+v.value,0);if(o===0)return`
    ...
    `;let l=0;const c=i.map(([r,d])=>{const v=d.value/o*100,h=l;return l+=v,{name:r,percentage:v,color:d.color,start:h}}),p=c.map(r=>`${r.color} ${r.start}% ${r.start+r.percentage}%`).join(", ");return`
  ...
  `}function It(){const s=document.getElementById("add-modal"),e=document.getElementById("modal-close");s.addEventListener("click",a=>{a.target===s&&ee()}),e.addEventListener("click",ee);const t=s.querySelectorAll(".category-selector .type-option");t.forEach(a=>{a.addEventListener("click",()=>{S=a.dataset.category,t.forEach(n=>n.classList.remove("active")),a.classList.add("active"),document.getElementById("form-container").innerHTML=ke(),pe()})}),pe()}function pe(){const s=document.getElementById("btn-save");s&&s.addEventListener("click",St);const e=document.getElementById("input-currency"),t=document.getElementById("input-name");e&&t&&e.addEventListener("change",()=>{if(!t.value){const a=e.options[e.selectedIndex].text;t.value=a.split(" (")[0]}})}function St(){var d,v,h,f,E,A,T,L,M,F,j,G;const s=(v=(d=document.getElementById("input-name"))==null?void 0:d.value)==null?void 0:v.trim(),e=(h=document.getElementById("input-type"))==null?void 0:h.value,t=(f=document.getElementById("input-currency"))==null?void 0:f.value,a=(A=(E=document.getElementById("input-details"))==null?void 0:E.value)==null?void 0:A.trim(),n=parseFloat((T=document.getElementById("input-value"))==null?void 0:T.value)||0,i=parseFloat((L=document.getElementById("input-amount"))==null?void 0:L.value)||0,o=parseFloat((M=document.getElementById("input-monthly"))==null?void 0:M.value)||0,l=(F=document.getElementById("input-date"))==null?void 0:F.value,c=(j=document.getElementById("input-time"))==null?void 0:j.value,p=(G=document.getElementById("input-repeat"))==null?void 0:G.value;if(!s){g.alert("Campo Obligatorio","Por favor ingresa un nombre para el elemento.");return}const r={name:s,type:e,currency:t,details:a};switch(S){case"passiveAsset":u.addPassiveAsset({...r,value:n,monthlyIncome:o});break;case"activeIncome":u.addActiveIncome({...r,amount:i});break;case"livingExpense":u.addLivingExpense({...r,amount:i});break;case"investmentAsset":u.addInvestmentAsset({...r,value:n});break;case"liability":u.addLiability({...r,amount:i,monthlyPayment:o});break;case"event":u.addEvent({title:s,date:l,time:c,repeat:p,category:e});break}ee()}function ee(){const s=document.getElementById("add-modal");s&&(s.classList.remove("active"),setTimeout(()=>s.remove(),300))}function xt(){const s=k.isSetup(),e=k.isBioEnabled();return`
    ...
    `}function At(s){const e=document.getElementById("auth-submit-btn"),t=document.getElementById("auth-bio-btn"),a=document.getElementById("auth-password"),n=async()=>{const i=a.value,o=document.getElementById("auth-confirm"),l=k.isSetup();try{let c;if(l)c=await k.unlock(i);else{if(!i||i.length<4)throw new Error("Contraseña demasiado corta");if(i!==o.value)throw new Error("Las contraseñas no coinciden");c=await k.setup(i)}await u.loadEncrypted(c),s()}catch(c){g.alert("Error",c.message)}};e==null||e.addEventListener("click",n),a==null||a.addEventListener("keypress",i=>{i.key==="Enter"&&n()}),t==null||t.addEventListener("click",async()=>{try{const i=await k.unlockWithBiometrics();await u.loadEncrypted(i),s()}catch(i){g.alert("Identificación",i.message)}}),k.isBioEnabled()&&setTimeout(async()=>{try{const i=await k.unlockWithBiometrics();await u.loadEncrypted(i),s()}catch{console.log("Auto-bio failed or cancelled")}},500)}const $t="modulepreload",Ct=function(s){return"/life-dashboard/"+s},me={},ve=function(e,t,a){let n=Promise.resolve();if(t&&t.length>0){document.getElementsByTagName("link");const o=document.querySelector("meta[property=csp-nonce]"),l=(o==null?void 0:o.nonce)||(o==null?void 0:o.getAttribute("nonce"));n=Promise.allSettled(t.map(c=>{if(c=Ct(c),c in me)return;me[c]=!0;const p=c.endsWith(".css"),r=p?'[rel="stylesheet"]':"";if(document.querySelector(`link[href="${c}"]${r}`))return;const d=document.createElement("link");if(d.rel=p?"stylesheet":$t,p||(d.as="script"),d.crossOrigin="",d.href=c,l&&d.setAttribute("nonce",l),document.head.appendChild(d),p)return new Promise((v,h)=>{d.addEventListener("load",v),d.addEventListener("error",()=>h(new Error(`Unable to preload CSS for ${c}`)))})}))}function i(o){const l=new Event("vite:preloadError",{cancelable:!0});if(l.payload=o,window.dispatchEvent(l),!l.defaultPrevented)throw o}return n.then(o=>{for(const l of o||[])l.status==="rejected"&&i(l.reason);return e().catch(i)})};function Tt(){const s=k.isBioEnabled(),e=$.hasToken();return`
    <div class="settings-page stagger-children" style="padding-bottom: 80px; max-width: 600px; margin: 0 auto;">
        ...
        <footer class="settings-footer">
            <p>Life Dashboard Pro v1.0.32</p>
            <p>© 2026 Privacy First Zero-Knowledge System</p>
        </footer>
    </div>
    `}function Lt(){var a,n,i,o,l,c,p;(a=document.getElementById("toggle-bio"))==null||a.addEventListener("change",async r=>{if(r.target.checked){const v=await g.prompt("Activar Biometría","Introduce tu contraseña maestra para confirmar:","Tu contraseña","password");if(v)try{await k.registerBiometrics(v),g.toast("Biometría activada correctamente")}catch(h){await g.alert("Error",h.message),r.target.checked=!1}else r.target.checked=!1}else localStorage.setItem("db_bio_enabled","false"),g.toast("Biometría desactivada","info")});const s=document.getElementById("btn-install-pwa");s&&setTimeout(()=>{if(window.deferredPrompt){const r=document.getElementById("install-pwa-card");r&&(r.style.display="block"),s.addEventListener("click",async()=>{if(!window.deferredPrompt)return;window.deferredPrompt.prompt();const{outcome:d}=await window.deferredPrompt.userChoice;if(d==="accepted"){g.toast("Instalando aplicación...");const v=document.getElementById("install-pwa-card");v&&(v.style.display="none")}window.deferredPrompt=null})}},1e3),(n=document.getElementById("sync-drive-btn"))==null||n.addEventListener("click",async()=>{const r=document.getElementById("sync-drive-btn"),d=$.hasToken(),v=r.innerHTML;try{if(r.innerHTML='<div class="loading-spinner-sm"></div>',r.style.pointerEvents="none",d){const h=k.getVaultKey();await $.pushData(u.getState(),h),g.toast("Bóveda actualizada en Drive")}else{await $.authenticate();const h=k.getVaultKey(),f=await $.pullData(h).catch(()=>null);if(f&&await g.confirm("Respaldo Encontrado","Hemos detectado una bóveda existente en Google Drive. ¿Qué deseas hacer con tus datos?","Recuperar de la Nube","Sobreescribir Nube")){u.setState(f),await u.saveState(),g.toast("Datos recuperados correctamente"),setTimeout(()=>window.location.reload(),800);return}await $.pushData(u.getState(),h),g.toast("Google Drive conectado y sincronizado")}typeof window.reRender=="function"&&window.reRender()}catch(h){if(console.error(h),h.message&&(h.message.includes("Contraseña incorrecta")||h.message.includes("corruptos"))){if(await g.confirm("Error de Decifrado","La contraseña actual no coincide con la del backup en la nube. ¿Deseas borrar los datos en Drive para empezar de cero?","Borrar Datos Nube","Cancelar"))try{await $.deleteBackup(),g.toast("Datos de Drive borrados");const E=k.getVaultKey();await $.pushData(u.getState(),E),g.toast("Google Drive sincronizado (Nueva Bóveda)")}catch{g.alert("Error","No se pudieron borrar los datos de Drive.")}}else g.alert("Error",h.message||(typeof h=="object"?JSON.stringify(h):"Error al conectar con Google"))}finally{r.innerHTML=v,r.style.pointerEvents="auto"}}),(i=document.getElementById("export-data-btn"))==null||i.addEventListener("click",async()=>{try{g.toast("Preparando archivo encriptado...","info");const r=u.getState(),d=k.getVaultKey(),{SecurityService:v}=await ve(async()=>{const{SecurityService:L}=await Promise.resolve().then(()=>ae);return{SecurityService:L}},void 0),h=await v.encrypt(r,d),f=new Blob([JSON.stringify(h)],{type:"application/octet-stream"}),E=URL.createObjectURL(f),A=document.createElement("a"),T=new Date().toISOString().split("T")[0];A.href=E,A.download=`life_dashboard_backup_${T}.bin`,document.body.appendChild(A),A.click(),document.body.removeChild(A),URL.revokeObjectURL(E),g.toast("Backup exportado correctamente")}catch(r){console.error("Export error:",r),g.alert("Error de Exportación","No se pudieron encriptar o descargar los datos.")}});const e=document.getElementById("import-backup-btn"),t=document.getElementById("import-backup-input");e==null||e.addEventListener("click",()=>{t==null||t.click()}),t==null||t.addEventListener("change",async r=>{var h;const d=(h=r.target.files)==null?void 0:h[0];if(!d)return;if(!await g.confirm("¿Importar Backup?","Esto sobreescribirá todos tus datos locales con los del archivo. ¿Deseas continuar?")){t.value="";return}try{const f=await d.text(),E=JSON.parse(f),A=k.getVaultKey(),{SecurityService:T}=await ve(async()=>{const{SecurityService:M}=await Promise.resolve().then(()=>ae);return{SecurityService:M}},void 0),L=await T.decrypt(E,A);if(L)u.setState(L),await u.saveState(),g.toast("Backup importado correctamente"),setTimeout(()=>window.location.reload(),1e3);else throw new Error("No se pudo descifrar el archivo")}catch(f){console.error("Import error:",f),g.alert("Error de Importación","El archivo no es válido o la contraseña no coincide con la usada para el backup.")}finally{t.value=""}}),(o=document.getElementById("btn-logout"))==null||o.addEventListener("click",async()=>{await g.confirm("¿Cerrar sesión?","El acceso quedará bloqueado hasta que introduzcas tu clave.")&&(k.logout(),window.location.reload())}),(l=document.getElementById("btn-force-update"))==null||l.addEventListener("click",async()=>{if(await g.confirm("¿Forzar Actualización?","Esto recargará la página y limpiará la caché para obtener la última versión.")){if(window.caches)try{const d=await caches.keys();for(let v of d)await caches.delete(v)}catch(d){console.error("Error clearing cache",d)}window.location.reload(!0)}}),(c=document.getElementById("btn-save-gemini"))==null||c.addEventListener("click",()=>{var d;const r=(d=document.getElementById("gemini-api-key"))==null?void 0:d.value;r!==void 0&&(localStorage.setItem("db_gemini_api_key",r.trim()),g.toast("API Key de Gemini guardada"))}),(p=document.getElementById("btn-factory-reset"))==null||p.addEventListener("click",async()=>{if(await g.hardConfirm("Borrar todos los datos","Esta acción eliminará permanentemente todos tus activos, ingresos, agenda y configuraciones de este dispositivo.","BORRAR")){if(localStorage.clear(),sessionStorage.clear(),window.indexedDB.databases&&(await window.indexedDB.databases()).forEach(v=>window.indexedDB.deleteDatabase(v.name)),navigator.serviceWorker){const d=await navigator.serviceWorker.getRegistrations();for(let v of d)v.unregister()}g.toast("Aplicación reseteada","info"),setTimeout(()=>{window.location.href=window.location.origin+"?reset="+Date.now()},1e3)}})}let R=localStorage.getItem("app_current_page")||"finance",I=localStorage.getItem("app_current_sub_page")||null;I==="null"&&(I=null);async function ge(){const s=k.getVaultKey();s?(await u.loadEncrypted(s),Ie()):Bt()}function Bt(){const s=document.getElementById("app");s.innerHTML=xt(),At(()=>{Ie()})}function Ie(){const s=document.getElementById("app");s.innerHTML=`
