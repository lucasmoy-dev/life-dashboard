var Se=Object.defineProperty;var xe=(s,e,t)=>e in s?Se(s,e,{enumerable:!0,configurable:!0,writable:!0,value:t}):s[e]=t;var X=(s,e,t)=>xe(s,typeof e!="symbol"?e+"":e,t);(function(){const e=document.createElement("link").relList;if(e&&e.supports&&e.supports("modulepreload"))return;for(const n of document.querySelectorAll('link[rel="modulepreload"]'))a(n);new MutationObserver(n=>{for(const i of n)if(i.type==="childList")for(const o of i.addedNodes)o.tagName==="LINK"&&o.rel==="modulepreload"&&a(o)}).observe(document,{childList:!0,subtree:!0});function t(n){const i={};return n.integrity&&(i.integrity=n.integrity),n.referrerPolicy&&(i.referrerPolicy=n.referrerPolicy),n.crossOrigin==="use-credentials"?i.credentials="include":n.crossOrigin==="anonymous"?i.credentials="omit":i.credentials="same-origin",i}function a(n){if(n.ep)return;n.ep=!0;const i=t(n);fetch(n.href,i)}})();const Ae="https://api.coingecko.com/api/v3/simple/price?ids=bitcoin,ethereum,ripple,kaspa&vs_currencies=eur,ars",$e="https://api.frankfurter.app/latest?from=EUR&to=USD,CHF,GBP,AUD";async function Ce(){const s={EUR:1};try{const t=await(await fetch(Ae)).json();s.BTC=t.bitcoin.eur,s.ETH=t.ethereum.eur,s.XRP=t.ripple.eur,s.KAS=t.kaspa.eur,t.bitcoin.ars&&t.bitcoin.eur&&(s.ARS=t.bitcoin.eur/t.bitcoin.ars);const n=await(await fetch($e)).json();s.USD=1/n.rates.USD,s.CHF=1/n.rates.CHF,s.GBP=1/n.rates.GBP,s.AUD=1/n.rates.AUD,s.GOLD=2100,s.SP500=4700}catch(e){console.error("Failed to fetch some prices:",e),s.USD=s.USD||.92,s.CHF=s.CHF||1.05,s.GBP=s.GBP||1.15,s.AUD=s.AUD||.6,s.ARS=s.ARS||.001}return s}class C{static async hash(e,t="salt_life_dashboard_2026"){const n=new TextEncoder().encode(e+t),i=await crypto.subtle.digest("SHA-512",n);return Array.from(new Uint8Array(i)).map(l=>l.toString(16).padStart(2,"0")).join("")}static async deriveVaultKey(e){return await this.hash(e,"vault_v4_dashboard_key")}static async deriveKey(e,t){const a=new TextEncoder,n=await crypto.subtle.importKey("raw",a.encode(e),{name:"PBKDF2"},!1,["deriveKey"]);return await crypto.subtle.deriveKey({name:"PBKDF2",salt:a.encode(t),iterations:25e4,hash:"SHA-512"},n,{name:"AES-GCM",length:256},!1,["encrypt","decrypt"])}static async encrypt(e,t){try{const a=crypto.getRandomValues(new Uint8Array(16)),n=crypto.getRandomValues(new Uint8Array(12)),i=await this.deriveKey(t,this.bufToBase64(a)),o=typeof e=="string"?e:JSON.stringify(e),l=new TextEncoder().encode(o),c=await crypto.subtle.encrypt({name:"AES-GCM",iv:n},i,l);return{payload:this.bufToBase64(new Uint8Array(c)),iv:this.bufToBase64(n),salt:this.bufToBase64(a),v:"5.0"}}catch(a){throw console.error("[Security] Encryption failed:",a),new Error("No se pudo encriptar la información")}}static async decrypt(e,t){try{if(!e||!e.payload||!e.iv||!e.salt)throw new Error("Formato de datos encriptados inválido");const{payload:a,iv:n,salt:i}=e,o=await this.deriveKey(t,i),l=await crypto.subtle.decrypt({name:"AES-GCM",iv:this.base64ToBuf(n)},o,this.base64ToBuf(a)),c=new TextDecoder().decode(l);try{return JSON.parse(c)}catch{return c}}catch(a){throw console.error("[Security] Decryption failed:",a),new Error("Contraseña incorrecta o datos corruptos")}}static bufToBase64(e){return btoa(String.fromCharCode(...new Uint8Array(e)))}static base64ToBuf(e){return new Uint8Array(atob(e).split("").map(t=>t.charCodeAt(0)))}}const Le=Object.freeze(Object.defineProperty({__proto__:null,SecurityService:C},Symbol.toStringTag,{value:"Module"})),S={MASTER_HASH:"db_master_hash",VAULT_KEY:"db_vault_key",BIO_ENABLED:"db_bio_enabled"};class w{static isSetup(){return!!localStorage.getItem(S.MASTER_HASH)}static async setup(e){const t=await C.hash(e),a=await C.deriveVaultKey(e);return localStorage.setItem(S.MASTER_HASH,t),sessionStorage.setItem(S.VAULT_KEY,a),a}static async unlock(e){const t=await C.hash(e),a=localStorage.getItem(S.MASTER_HASH);if(t===a){const n=await C.deriveVaultKey(e);return sessionStorage.setItem(S.VAULT_KEY,n),n}throw new Error("Contraseña incorrecta")}static async registerBiometrics(e){await this.unlock(e);const t=sessionStorage.getItem(S.VAULT_KEY);if(!window.PublicKeyCredential)throw new Error("Biometría no soportada en este dispositivo");try{const a=crypto.getRandomValues(new Uint8Array(32));return await navigator.credentials.create({publicKey:{challenge:a,rp:{name:"Life Dashboard",id:window.location.hostname},user:{id:crypto.getRandomValues(new Uint8Array(16)),name:"user",displayName:"User"},pubKeyCredParams:[{alg:-7,type:"public-key"}],timeout:6e4,authenticatorSelection:{authenticatorAttachment:"platform"},attestation:"none"}}),localStorage.setItem(S.BIO_ENABLED,"true"),localStorage.setItem(S.VAULT_KEY,t),!0}catch(a){throw console.error("Biometric setup failed:",a),new Error("Error al configurar biometría")}}static async unlockWithBiometrics(){if(!(localStorage.getItem(S.BIO_ENABLED)==="true"))throw new Error("Biometría no activada");try{const t=crypto.getRandomValues(new Uint8Array(32));await navigator.credentials.get({publicKey:{challenge:t,rpId:window.location.hostname,userVerification:"required",timeout:6e4}});const a=localStorage.getItem(S.VAULT_KEY);if(a)return sessionStorage.setItem(S.VAULT_KEY,a),a;throw new Error("Llave no encontrada. Usa contraseña.")}catch(t){throw console.error("Biometric auth failed:",t),new Error("Fallo de identificación biométrica")}}static logout(){sessionStorage.removeItem(S.VAULT_KEY)}static getVaultKey(){return sessionStorage.getItem(S.VAULT_KEY)}static isBioEnabled(){return localStorage.getItem(S.BIO_ENABLED)==="true"}}const Te="974464877836-721dprai6taijtuufmrkh438q68e97sp.apps.googleusercontent.com",Be="https://www.googleapis.com/auth/drive.file";class x{static hasToken(){return!!this.accessToken}static async init(){return new Promise(e=>{const t=()=>{window.gapi&&window.google?gapi.load("client",async()=>{await gapi.client.init({discoveryDocs:["https://www.googleapis.com/discovery/v1/apis/drive/v3/rest"]}),this.tokenClient=google.accounts.oauth2.initTokenClient({client_id:Te,scope:Be,callback:a=>{if(a.error)throw a;this.accessToken=a.access_token,e(!0)}}),e(!0)}):setTimeout(t,100)};t()})}static async authenticate(){return this.tokenClient||await this.init(),new Promise((e,t)=>{this.tokenClient.callback=a=>{a.error&&t(a),this.accessToken=a.access_token,sessionStorage.setItem("drive_access_token",a.access_token),e(a.access_token)},this.tokenClient.requestAccessToken({prompt:"consent"})})}static async getOrCreateFolderPath(e){var n;(n=gapi.client)!=null&&n.drive||await this.init();const t=e.split("/").filter(i=>i);let a="root";for(const i of t){const o=`name = '${i}' and mimeType = 'application/vnd.google-apps.folder' and '${a}' in parents and trashed = false`,c=(await gapi.client.drive.files.list({q:o,fields:"files(id, name)"})).result.files;if(c.length>0)a=c[0].id;else{const u={name:i,mimeType:"application/vnd.google-apps.folder",parents:[a]};a=(await gapi.client.drive.files.create({resource:u,fields:"id"})).result.id}}return a}static async pushData(e,t){try{if(!this.accessToken)throw new Error("Cloud not connected");console.log("[Drive] Pushing full encrypted data...");const a=await this.getOrCreateFolderPath("/backup/life-dashboard/"),n=await C.encrypt(e,t),i="dashboard_vault_v5.bin",o=`name = '${i}' and '${a}' in parents and trashed = false`,c=(await gapi.client.drive.files.list({q:o,fields:"files(id)"})).result.files,u=new Blob([JSON.stringify(n)],{type:"application/json"});if(c.length>0){const r=c[0].id,d=await fetch(`https://www.googleapis.com/upload/drive/v3/files/${r}?uploadType=media`,{method:"PATCH",headers:{Authorization:`Bearer ${this.accessToken}`},body:u});if(!d.ok)throw new Error(`Drive update failed: ${d.status}`)}else{const r={name:i,parents:[a]},d=new FormData;d.append("metadata",new Blob([JSON.stringify(r)],{type:"application/json"})),d.append("file",u);const v=await fetch("https://www.googleapis.com/upload/drive/v3/files?uploadType=multipart",{method:"POST",headers:{Authorization:`Bearer ${this.accessToken}`},body:d});if(!v.ok)throw new Error(`Drive creation failed: ${v.status}`)}return!0}catch(a){throw console.error("[Drive] Push failed:",a),a}}static async pullData(e){var t;try{if(!this.accessToken)throw new Error("Cloud not connected");(t=gapi.client)!=null&&t.drive||await this.init(),console.log("[Drive] Pulling data...");const i=`name = 'dashboard_vault_v5.bin' and '${await this.getOrCreateFolderPath("/backup/life-dashboard/")}' in parents and trashed = false`,l=(await gapi.client.drive.files.list({q:i,fields:"files(id, name)"})).result.files;if(l.length===0)return null;const c=l[0].id,r=await(await fetch(`https://www.googleapis.com/drive/v3/files/${c}?alt=media`,{headers:{Authorization:`Bearer ${this.accessToken}`}})).json();return await C.decrypt(r,e)}catch(a){throw console.error("[Drive] Pull failed:",a),a}}static async deleteBackup(){var e;try{if(!this.accessToken)throw new Error("Cloud not connected");(e=gapi.client)!=null&&e.drive||await this.init();const n=`name = 'dashboard_vault_v5.bin' and '${await this.getOrCreateFolderPath("/backup/life-dashboard/")}' in parents and trashed = false`,o=(await gapi.client.drive.files.list({q:n,fields:"files(id)"})).result.files;if(o.length>0){const l=o[0].id;return await gapi.client.drive.files.delete({fileId:l}),console.log("[Drive] Backup deleted successfully"),!0}return!1}catch(t){throw console.error("[Drive] Deletion failed:",t),t}}}X(x,"tokenClient",null),X(x,"accessToken",sessionStorage.getItem("drive_access_token")||null);const ae={wallet:'<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M21 12V7H5a2 2 0 0 1 0-4h14v4"/><path d="M3 5v14a2 2 0 0 0 2 2h16v-5"/><path d="M18 12a2 2 0 0 0 0 4h4v-4Z"/></svg>',target:'<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><circle cx="12" cy="12" r="10"/><circle cx="12" cy="12" r="6"/><circle cx="12" cy="12" r="2"/></svg>',calendar:'<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><rect width="18" height="18" x="3" y="4" rx="2" ry="2"/><line x1="16" x2="16" y1="2" y2="6"/><line x1="8" x2="8" y1="2" y2="6"/><line x1="3" x2="21" y1="10" y2="10"/></svg>',heart:'<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M19 14c1.49-1.46 3-3.21 3-5.5A5.5 5.5 0 0 0 16.5 3c-1.76 0-3 .5-4.5 2-1.5-1.5-2.74-2-4.5-2A5.5 5.5 0 0 0 2 8.5c0 2.3 1.5 4.05 3 5.5l7 7Z"/></svg>',settings:'<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M12.22 2h-.44a2 2 0 0 0-2 2v.18a2 2 0 0 1-1 1.73l-.43.25a2 2 0 0 1-2 0l-.15-.08a2 2 0 0 0-2.73.73l-.22.38a2 2 0 0 0 .73 2.73l.15.1a2 2 0 0 1 1 1.72v.51a2 2 0 0 1-1 1.74l-.15.09a2 2 0 0 0-.73 2.73l.22.38a2 2 0 0 0 2.73.73l.15-.08a2 2 0 0 1 2 0l.43.25a2 2 0 0 1 1 1.73V20a2 2 0 0 0 2 2h.44a2 2 0 0 0 2-2v-.18a2 2 0 0 1 1-1.73l.43-.25a2 2 0 0 1 2 0l.15.08a2 2 0 0 0 2.73-.73l.22-.39a2 2 0 0 0-.73-2.73l-.15-.08a2 2 0 0 1-1-1.74v-.5a2 2 0 0 1 1-1.74l.15-.09a2 2 0 0 0 .73-2.73l-.22-.38a2 2 0 0 0-2.73-.73l-.15.08a2 2 0 0 1-2 0l-.43-.25a2 2 0 0 1-1-1.73V4a2 2 0 0 0-2-2z"/><circle cx="12" cy="12" r="3"/></svg>',building:'<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><rect width="16" height="20" x="4" y="2" rx="2" ry="2"/><path d="M9 22v-4h6v4"/><path d="M8 6h.01"/><path d="M16 6h.01"/><path d="M12 6h.01"/><path d="M12 10h.01"/><path d="M12 14h.01"/><path d="M16 10h.01"/><path d="M16 14h.01"/><path d="M8 10h.01"/><path d="M8 14h.01"/></svg>',home:'<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="m3 9 9-7 9 7v11a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2z"/><polyline points="9 22 9 12 15 12 15 22"/></svg>',trendingUp:'<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><polyline points="22 7 13.5 15.5 8.5 10.5 2 17"/><polyline points="16 7 22 7 22 13"/></svg>',bitcoin:'<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M11.767 19.089c4.924.868 6.14-6.025 1.216-6.894m-1.216 6.894L5.86 18.047m5.908 1.042-.347 1.97m1.563-8.864c4.924.869 6.14-6.025 1.215-6.893m-1.215 6.893-3.94-.694m5.155-6.2L8.29 4.26m5.908 1.042.348-1.97M7.48 20.364l3.126-17.727"/></svg>',dollarSign:'<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><line x1="12" x2="12" y1="2" y2="22"/><path d="M17 5H9.5a3.5 3.5 0 0 0 0 7h5a3.5 3.5 0 0 1 0 7H6"/></svg>',car:'<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M19 17h2c.6 0 1-.4 1-1v-3c0-.9-.7-1.7-1.5-1.9C18.7 10.6 16 10 16 10s-1.3-1.4-2.2-2.3c-.5-.4-1.1-.7-1.8-.7H5c-.6 0-1.1.4-1.4.9l-1.5 2.8C1.4 11.3 1 12.1 1 13v3c0 .6.4 1 1 1h2"/><circle cx="7" cy="17" r="2"/><path d="M9 17h6"/><circle cx="17" cy="17" r="2"/></svg>',creditCard:'<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><rect width="20" height="14" x="2" y="5" rx="2"/><line x1="2" x2="22" y1="10" y2="10"/></svg>',landmark:'<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><line x1="3" x2="21" y1="22" y2="22"/><line x1="6" x2="6" y1="18" y2="11"/><line x1="10" x2="10" y1="18" y2="11"/><line x1="14" x2="14" y1="18" y2="11"/><line x1="18" x2="18" y1="18" y2="11"/><polygon points="12 2 20 7 4 7"/></svg>',plus:'<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M5 12h14"/><path d="M12 5v14"/></svg>',x:'<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M18 6 6 18"/><path d="m6 6 12 12"/></svg>',trash:'<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M3 6h18"/><path d="M19 6v14c0 1-1 2-2 2H7c-1 0-2-1-2-2V6"/><path d="M8 6V4c0-1 1-2 2-2h4c1 0 2 1 2 2v2"/></svg>',edit:'<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M17 3a2.85 2.83 0 1 1 4 4L7.5 20.5 2 22l1.5-5.5Z"/><path d="m15 5 4 4"/></svg>',chevronRight:'<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="m9 18 6-6-6-6"/></svg>',chevronLeft:'<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="m15 18-6-6 6-6"/></svg>',calculator:'<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><rect width="16" height="20" x="4" y="2" rx="2"/><line x1="8" x2="16" y1="6" y2="6"/><line x1="16" x2="16" y1="14" y2="18"/><path d="M16 10h.01"/><path d="M12 10h.01"/><path d="M8 10h.01"/><path d="M12 14h.01"/><path d="M8 14h.01"/><path d="M12 18h.01"/><path d="M8 18h.01"/></svg>',arrowUpRight:'<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M7 7h10v10"/><path d="M7 17 17 7"/></svg>',arrowDownRight:'<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="m7 7 10 10"/><path d="M17 7v10H7"/></svg>',piggyBank:'<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M19 5c-1.5 0-2.8 1.4-3 2-3.5-1.5-11-.3-11 5 0 1.8 0 3 2 4.5V20h4v-2h3v2h4v-4c1-.5 1.7-1 2-2h2v-4h-2c0-1-.5-1.5-1-2V5z"/><path d="M2 9v1c0 1.1.9 2 2 2h1"/><path d="M16 11h.01"/></svg>',receipt:'<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M4 2v20l2-1 2 1 2-1 2 1 2-1 2 1 2-1 2 1V2l-2 1-2-1-2 1-2-1-2 1-2-1-2 1Z"/><path d="M16 8h-6a2 2 0 1 0 0 4h4a2 2 0 1 1 0 4H8"/><path d="M12 17.5v-11"/></svg>',coins:'<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><circle cx="8" cy="8" r="6"/><path d="M18.09 10.37A6 6 0 1 1 10.34 18"/><path d="M7 6h1v4"/><path d="m16.71 13.88.7.71-2.82 2.82"/></svg>',scale:'<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="m16 16 3-8 3 8c-.87.65-1.92 1-3 1s-2.13-.35-3-1Z"/><path d="m2 16 3-8 3 8c-.87.65-1.92 1-3 1s-2.13-.35-3-1Z"/><path d="M7 21h10"/><path d="M12 3v18"/><path d="M3 7h2c2 0 5-1 7-2 2 1 5 2 7 2h2"/></svg>',briefcase:'<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><rect width="20" height="14" x="2" y="7" rx="2" ry="2"/><path d="M16 21V5a2 2 0 0 0-2-2h-4a2 2 0 0 0-2 2v16"/></svg>',zap:'<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><polygon points="13 2 3 14 12 14 11 22 21 10 12 10 13 2"/></svg>',download:'<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M21 15v4a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2v-4"/><polyline points="7 10 12 15 17 10"/><line x1="12" x2="12" y1="15" y2="3"/></svg>',downloadCloud:'<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M4 14.899A7 7 0 1 1 15.71 8h1.79a4.5 4.5 0 0 1 2.5 8.242"/><path d="M12 12v9"/><path d="m8 17 4 4 4-4"/></svg>',cloud:'<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M17.5 19H9a7 7 0 1 1 6.71-9h1.79a4.5 4.5 0 1 1 0 9Z"/></svg>',shield:'<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M12 22s8-4 8-10V5l-8-3-8 3v7c0 6 8 10 8 10z"/></svg>',link:'<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M10 13a5 5 0 0 0 7.54.54l3-3a5 5 0 0 0-7.07-7.07l-1.72 1.71"/><path d="M14 11a5 5 0 0 0-7.54-.54l-3 3a5 5 0 0 0 7.07 7.07l1.71-1.71"/></svg>',refreshCw:'<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M3 12a9 9 0 0 1 9-9 9.75 9.75 0 0 1 6.74 2.74L21 8"/><path d="M21 3v5h-5"/><path d="M21 12a9 9 0 0 1-9 9 9.75 9.75 0 0 1-6.74-2.74L3 16"/><path d="M8 16H3v5"/></svg>',lock:'<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><rect width="18" height="11" x="3" y="11" rx="2" ry="2"/><path d="M7 11V7a5 5 0 0 1 10 0v4"/></svg>',logOut:'<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M9 21H5a2 2 0 0 1-2-2V5a2 2 0 0 1 2-2h4"/><polyline points="16 17 21 12 16 7"/><line x1="21" x2="9" y1="12" y2="12"/></svg>',package:'<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="m7.5 4.27 9 5.15"/><path d="M21 8a2 2 0 0 0-1-1.73l-7-4a2 2 0 0 0-2 0l-7 4A2 2 0 0 0 3 8v8a2 2 0 0 0 1 1.73l7 4a2 2 0 0 0 2 0l7-4A2 2 0 0 0 21 16Z"/><path d="m3.3 7 8.7 5 8.7-5"/><path d="M12 22V12"/></svg>'};function m(s,e=""){return(ae[s]||ae.package).replace("<svg",`<svg class="${e}"`)}class Pe{constructor(){this.toastContainer=null,this._initToastContainer()}_initToastContainer(){document.getElementById("toast-container")||(this.toastContainer=document.createElement("div"),this.toastContainer.id="toast-container",this.toastContainer.className="toast-container",document.body.appendChild(this.toastContainer))}toast(e,t="success",a=3e3){const n=document.createElement("div");n.className=`toast toast-${t} stagger-in`;const i=t==="success"?"check":t==="error"?"alertCircle":"info";n.innerHTML=`
    ...
    `).join("");return e+t}function ie(s){const e=document.querySelectorAll(".nav-item");e.forEach(t=>{t.addEventListener("click",()=>{const a=t.dataset.nav;e.forEach(n=>n.classList.remove("active")),t.classList.add("active"),s&&s(a)})})}function y(s,e="$"){const t=Math.abs(s);let a=0,n=0;e==="₿"?(a=4,n=6):(e==="$"||e==="€"||e==="£"||e==="Fr")&&(a=0,n=2);const i=new Intl.NumberFormat("en-US",{minimumFractionDigits:a,maximumFractionDigits:n}).format(t);return`${s<0?"-":""}${e}${i}`}function De(s,e="$"){const t=Math.abs(s);let a;return t>=1e6?a=(t/1e6).toFixed(1)+"M":t>=1e3?a=(t/1e3).toFixed(1)+"K":a=t.toString(),`${s<0?"-":""}${e}${a}`}function Y(s){return s==null?"0.0%":`${s>=0?"+":""}${s.toFixed(1)}%`}const _e={passive:{label:"Activo Pasivo",storeKey:"passiveAssets",updateMethod:"updatePassiveAsset",deleteMethod:"deletePassiveAsset",fields:["value","monthlyIncome"]},investment:{label:"Activo de Inversión",storeKey:"investmentAssets",updateMethod:"updateInvestmentAsset",deleteMethod:"deleteInvestmentAsset",fields:["value"]},liability:{label:"Pasivo/Deuda",storeKey:"liabilities",updateMethod:"updateLiability",deleteMethod:"deleteLiability",fields:["amount","monthlyPayment"]},activeIncome:{label:"Ingreso Activo",storeKey:"activeIncomes",updateMethod:"updateActiveIncome",deleteMethod:"deleteActiveIncome",fields:["amount"]},livingExpense:{label:"Gasto de Vida",storeKey:"livingExpenses",updateMethod:"updateLivingExpense",deleteMethod:"deleteLivingExpense",fields:["amount"]}};let V=null,H=null;function ge(s,e){const t=_e[e];if(!t){console.error("Unknown category:",e);return}const i=p.getState()[t.storeKey].find(l=>l.id===s);if(!i){console.error("Item not found:",s);return}V=i,H=e;const o=document.createElement("div");o.className="modal-overlay",o.id="edit-modal",o.innerHTML=Ue(i,t),document.body.appendChild(o),requestAnimationFrame(()=>{o.classList.add("active")}),Ne(t)}const Oe=[{value:"EUR",label:"Euro (€)"},{value:"USD",label:"Dólar ($)"},{value:"BTC",label:"Bitcoin (BTC)"},{value:"ETH",label:"Ethereum (ETH)"},{value:"XRP",label:"Ripple (XRP)"},{value:"GOLD",label:"Oro (XAU)"},{value:"SP500",label:"S&P 500 (SPX)"}];function Ue(s,e){p.getState().currencySymbol;let t="";return e.fields.includes("value")&&(t+=`
  ...
  `}function Ne(s){const e=document.getElementById("edit-modal"),t=document.getElementById("edit-modal-close"),a=document.getElementById("btn-update"),n=document.getElementById("btn-delete");e.addEventListener("click",i=>{i.target===e&&W()}),t.addEventListener("click",W),a.addEventListener("click",()=>Fe(s)),n.addEventListener("click",()=>je(s))}function Fe(s){var c,u,r,d,v,b,k,$;const e=(u=(c=document.getElementById("edit-name"))==null?void 0:c.value)==null?void 0:u.trim(),t=(r=document.getElementById("edit-currency"))==null?void 0:r.value,a=(v=(d=document.getElementById("edit-details"))==null?void 0:d.value)==null?void 0:v.trim(),n=parseFloat((b=document.getElementById("edit-value"))==null?void 0:b.value)||0,i=parseFloat((k=document.getElementById("edit-amount"))==null?void 0:k.value)||0,o=parseFloat(($=document.getElementById("edit-monthly"))==null?void 0:$.value)||0;if(!e){g.alert("Requerido","El nombre es obligatorio para guardar los cambios.");return}const l={name:e,currency:t,details:a};s.fields.includes("value")&&(l.value=n),s.fields.includes("amount")&&(l.amount=i),s.fields.includes("monthlyIncome")&&(l.monthlyIncome=o),s.fields.includes("monthlyPayment")&&(l.monthlyPayment=o),p[s.updateMethod](V.id,l),W()}function je(s){g.confirm("¿Eliminar?",`¿Estás seguro de que quieres borrar "${V.name}"? Esta acción no se puede deshacer.`).then(e=>{e&&(p[s.deleteMethod](V.id),g.toast("Eliminado correctamente","info"),W())})}function W(){const s=document.getElementById("edit-modal");s&&(s.classList.remove("active"),setTimeout(()=>s.remove(),300)),V=null,H=null}function oe(){const s=p.getState(),e=s.currencySymbol,t=p.getPassiveIncome(),a=p.getLivingExpenses(),n=p.getNetPassiveIncome(),i=p.getInvestmentAssetsValue(),o=p.getTotalLiabilities(),l=p.getNetWorth(),c=p.getAllIncomes(),u=p.getAllExpenses(),r=p.getNetIncome();return`
  ...
  `}function Ge(s){const e=[...s.passiveAssets,...s.investmentAssets],t=s.liabilities;if(e.length===0)return"";const a={Bitcoin:{value:0,color:"#f59e0b"},Altcoins:{value:0,color:"#6366f1"},Inmuebles:{value:0,color:"#a855f7"},Bolsa:{value:0,color:"#00d4aa"},Oro:{value:0,color:"#fbbf24"},"Otros/Efe.":{value:0,color:"#94a3b8"}};e.forEach(r=>{const d=p.convertValue(r.value||0,r.currency||"EUR");r.currency==="BTC"?a.Bitcoin.value+=d:r.currency==="ETH"||r.currency==="XRP"||r.type==="crypto"?a.Altcoins.value+=d:r.type==="property"||r.type==="rental"?a.Inmuebles.value+=d:r.type==="stocks"||r.type==="etf"||r.currency==="SP500"?a.Bolsa.value+=d:r.currency==="GOLD"?a.Oro.value+=d:a["Otros/Efe."].value+=d});const n=t.filter(r=>r.type==="mortgage").reduce((r,d)=>r+p.convertValue(d.amount||0,d.currency||"EUR"),0);a.Inmuebles.value=Math.max(0,a.Inmuebles.value-n),s.hideRealEstate&&(a.Inmuebles.value=0);const i=Object.entries(a).filter(([r,d])=>d.value>0).sort((r,d)=>d[1].value-r[1].value),o=i.reduce((r,[d,v])=>r+v.value,0);if(o===0)return`
    ...
    `;let l=0;const c=i.map(([r,d])=>{const v=d.value/o*100,b=l;return l+=v,{name:r,percentage:v,color:d.color,start:b}}),u=c.map(r=>`${r.color} ${r.start}% ${r.start+r.percentage}%`).join(", ");return`
  ...
  `}function He(s){const e=[...s.passiveAssets.map(a=>({...a,category:"passive"})),...s.investmentAssets.map(a=>({...a,category:"investment"})),...s.liabilities.map(a=>({...a,category:"liability"}))];if(e.length===0)return`
    ...
    <div class="asset-list">
      ${e.map(a=>{const n=Ke(a.currency||a.type),i=Ve(a.currency||a.type),o=a.category==="liability",l=a.value||a.amount||0,c=p.convertValue(l,a.currency||"EUR");let u="";if(a.currency!==s.currency){const d={EUR:"€",USD:"$",BTC:"₿",ETH:"Ξ",XRP:"✕",GOLD:"oz",SP500:"pts",CHF:"Fr",GBP:"£",AUD:"A$",ARS:"$"}[a.currency]||a.currency;u=`<div class="asset-original-value">${l} ${d}</div>`}return`
  ...
  `}function Ke(s){return{property:"property",rental:"property",stocks:"stocks",etf:"stocks",SP500:"stocks",crypto:"crypto",BTC:"crypto",ETH:"crypto",XRP:"crypto",GOLD:"investment",cash:"cash",USD:"cash",EUR:"cash",savings:"cash",vehicle:"vehicle",debt:"debt",loan:"debt",mortgage:"debt",creditcard:"debt"}[s]||"cash"}function Ve(s){return{property:"building",rental:"building",stocks:"trendingUp",etf:"trendingUp",SP500:"trendingUp",crypto:"bitcoin",BTC:"bitcoin",ETH:"bitcoin",XRP:"bitcoin",GOLD:"package",cash:"dollarSign",USD:"dollarSign",EUR:"dollarSign",savings:"piggyBank",vehicle:"car",debt:"creditCard",loan:"landmark",mortgage:"home",creditcard:"creditCard"}[s]||"dollarSign"}function re(){document.querySelectorAll(".asset-item").forEach(a=>{a.addEventListener("click",()=>{const n=a.dataset.id,i=a.dataset.category;ge(n,i)})});const e=document.getElementById("toggle-real-estate");e&&e.addEventListener("change",()=>{p.toggleRealEstate()});const t=document.getElementById("display-currency-select");t&&t.addEventListener("change",a=>{p.setCurrency(a.target.value)})}let L=10,M=7,R=null,D=null;function ze(){const e=p.getState().currencySymbol,t=p.getNetWorth(),n=p.getNetIncome()*12,i=R!==null?R:t,o=D!==null?D:n,l=he(i,o,M,L);return`
    <div class="compound-page stagger-children" style="padding-bottom: 80px;">
      ...
      <div class="card highlight-card">
        ...
        <div class="highlight-value" id="future-value">${y(l.finalValue,e)}</div>
      ...
      <div class="card">
        ...
        <div class="stat-row">
          ...
          <span class="stat-value ${l.totalContributions>=0?"positive":"negative"}" id="total-contributed">${y(l.totalContributions,e)}</span>
        </div>
        ...
        <div class="stat-row">
          ...
          <span class="stat-value" style="color: var(--accent-secondary);" id="total-interest">${y(l.totalInterest,e)}</span>
        </div>
        ...
        <div class="stat-row" style="padding-top: var(--spacing-md); border-top: 1px solid rgba(255,255,255,0.1); margin-top: var(--spacing-sm);">
          ...
          <span class="stat-value positive" style="font-size: 20px;" id="final-value-breakdown">${y(l.finalValue,e)}</span>
        </div>
      </div>
      ...
      ${s.filter((i,o)=>(o+1)%a===0||o===s.length-1).map(i=>{const o=t>0?Math.abs(i.endBalance)/t*100:0;return`
          <div class="chart-bar-container">
            <div class="chart-bar ${i.endBalance<0?"negative":""}" style="height: ${Math.max(o,5)}%;">
              <span class="chart-bar-value">${De(i.endBalance,e)}</span>
            </div>
            <span class="chart-bar-label">Año ${i.year}</span>
          </div>
        `}).join("")}
    </div>
  `}function be(s,e){const t=[];for(let a=0;a<s.length;a++){const n=s[a];(a<5||(a+1)%5===0||a===s.length-1)&&t.push(n)}return`
  ...
  `}function Ye(s){const e=document.getElementById("back-to-finance"),t=document.getElementById("rate-slider"),a=document.getElementById("years-slider"),n=document.getElementById("principal-input"),i=document.getElementById("contribution-input"),o=document.getElementById("reset-principal"),l=document.getElementById("reset-contribution");e&&e.addEventListener("click",s),n&&n.addEventListener("input",c=>{R=parseFloat(c.target.value)||0,O()}),i&&i.addEventListener("input",c=>{D=parseFloat(c.target.value)||0,O()}),o&&o.addEventListener("click",()=>{R=null;const c=p.getNetWorth();n.value=c,O()}),l&&l.addEventListener("click",()=>{D=null;const c=p.getNetIncome()*12;i.value=c,O()}),t&&t.addEventListener("input",c=>{M=parseFloat(c.target.value),document.getElementById("rate-value").textContent=`${M}%`,O()}),a&&a.addEventListener("input",c=>{L=parseInt(c.target.value),document.getElementById("years-value").textContent=`${L} años`,O()})}function O(){const e=p.getState().currencySymbol,t=R!==null?R:p.getNetWorth(),a=D!==null?D:p.getNetIncome()*12,n=he(t,a,M,L),i=document.getElementById("future-value"),o=document.getElementById("future-value-title"),l=document.getElementById("growth-label"),c=document.getElementById("initial-capital"),u=document.getElementById("total-contributed"),r=document.getElementById("total-interest"),d=document.getElementById("final-value-breakdown"),v=document.getElementById("projection-chart"),b=document.getElementById("projection-table");i&&(i.textContent=y(n.finalValue,e)),o&&(o.textContent=`Valor Futuro en ${L} años`),l&&(l.innerHTML=`${n.totalGrowth>=0?"📈":"📉"} ${n.growthMultiple.toFixed(1)}x tu capital inicial`),c&&(c.textContent=y(t,e)),u&&(u.textContent=y(n.totalContributions,e),u.className=`stat-value ${n.totalContributions>=0?"positive":"negative"}`),r&&(r.textContent=y(n.totalInterest,e)),d&&(d.textContent=y(n.finalValue,e)),v&&(v.innerHTML=ye(n.yearlyBreakdown,e)),b&&(b.innerHTML=be(n.yearlyBreakdown,e))}function qe(){L=10,M=7,R=null,D=null}const We="https://api.coingecko.com/api/v3",h={STOCKS:"Stocks & Índices",CRYPTO_MAJORS:"Cripto (Principales)",CRYPTO_ALTS:"Cripto (Altcoins)",COMMODITIES:"Materias Primas"},U=[{id:"sp500",name:"S&P 500",symbol:"SPX",category:h.STOCKS,yahooId:"%5EGSPC"},{id:"nasdaq100",name:"Nasdaq 100",symbol:"NDX",category:h.STOCKS,yahooId:"%5EIXIC"},{id:"msciworld",name:"MSCI World ETF",symbol:"URTH",category:h.STOCKS,yahooId:"URTH"},{id:"microsoft",name:"Microsoft",symbol:"MSFT",category:h.STOCKS,yahooId:"MSFT"},{id:"tesla",name:"Tesla",symbol:"TSLA",category:h.STOCKS,yahooId:"TSLA"},{id:"apple",name:"Apple",symbol:"AAPL",category:h.STOCKS,yahooId:"AAPL"},{id:"amazon",name:"Amazon",symbol:"AMZN",category:h.STOCKS,yahooId:"AMZN"},{id:"nvidia",name:"Nvidia",symbol:"NVDA",category:h.STOCKS,yahooId:"NVDA"},{id:"google",name:"Google",symbol:"GOOGL",category:h.STOCKS,yahooId:"GOOGL"},{id:"meta",name:"Meta",symbol:"META",category:h.STOCKS,yahooId:"META"},{id:"oracle",name:"Oracle",symbol:"ORCL",category:h.STOCKS,yahooId:"ORCL"},{id:"netflix",name:"Netflix",symbol:"NFLX",category:h.STOCKS,yahooId:"NFLX"},{id:"ypf",name:"YPF",symbol:"YPF",category:h.STOCKS,yahooId:"YPF"},{id:"ibex35",name:"IBEX 35",symbol:"IBEX",category:h.STOCKS,yahooId:"%5EIBEX"},{id:"gold",name:"Oro",symbol:"XAU",category:h.COMMODITIES,cgId:"pax-gold"},{id:"silver",name:"Plata",symbol:"XAG",category:h.COMMODITIES,cgId:"tether-gold"},{id:"copper",name:"Cobre",symbol:"HG",category:h.COMMODITIES,yahooId:"HG=F"},{id:"bitcoin",name:"Bitcoin",symbol:"BTC",cgId:"bitcoin",category:h.CRYPTO_MAJORS},{id:"ethereum",name:"Ethereum",symbol:"ETH",cgId:"ethereum",category:h.CRYPTO_MAJORS},{id:"ripple",name:"XRP",symbol:"XRP",cgId:"ripple",category:h.CRYPTO_MAJORS},{id:"solana",name:"Solana",symbol:"SOL",cgId:"solana",category:h.CRYPTO_MAJORS},{id:"cardano",name:"Cardano",symbol:"ADA",cgId:"cardano",category:h.CRYPTO_MAJORS},{id:"dogecoin",name:"Dogecoin",symbol:"DOGE",cgId:"dogecoin",category:h.CRYPTO_MAJORS},{id:"kaspa",name:"Kaspa",symbol:"KAS",cgId:"kaspa",category:h.CRYPTO_ALTS},{id:"litecoin",name:"Litecoin",symbol:"LTC",cgId:"litecoin",category:h.CRYPTO_ALTS},{id:"bitcoin-cash",name:"Bitcoin Cash",symbol:"BCH",cgId:"bitcoin-cash",category:h.CRYPTO_ALTS},{id:"monero",name:"Monero",symbol:"XMR",cgId:"monero",category:h.CRYPTO_ALTS},{id:"chainlink",name:"Chainlink",symbol:"LINK",cgId:"chainlink",category:h.CRYPTO_ALTS},{id:"stellar",name:"Stellar",symbol:"XLM",cgId:"stellar",category:h.CRYPTO_ALTS},{id:"sui",name:"Sui",symbol:"SUI",cgId:"sui",category:h.CRYPTO_ALTS},{id:"hbar",name:"Hedera",symbol:"HBAR",cgId:"hedera-hashgraph",category:h.CRYPTO_ALTS},{id:"aave",name:"Aave",symbol:"AAVE",cgId:"aave",category:h.CRYPTO_ALTS},{id:"bittensor",name:"Bittensor",symbol:"TAO",cgId:"bittensor",category:h.CRYPTO_ALTS},{id:"worldcoin",name:"Worldcoin",symbol:"WLD",cgId:"worldcoin-org",category:h.CRYPTO_ALTS},{id:"arbitrum",name:"Arbitrum",symbol:"ARB",cgId:"arbitrum",category:h.CRYPTO_ALTS},{id:"polygon",name:"Polygon",symbol:"POL",cgId:"polygon-ecosystem-token",category:h.CRYPTO_ALTS},{id:"optimism",name:"Optimism",symbol:"OP",cgId:"optimism",category:h.CRYPTO_ALTS},{id:"stacks",name:"Stacks",symbol:"STX",cgId:"blockstack",category:h.CRYPTO_ALTS},{id:"ondo",name:"Ondo",symbol:"ONDO",cgId:"ondo-finance",category:h.CRYPTO_ALTS},{id:"zcash",name:"Zcash",symbol:"ZEC",cgId:"zcash",category:h.CRYPTO_ALTS},{id:"dash",name:"Dash",symbol:"DASH",cgId:"dash",category:h.CRYPTO_ALTS},{id:"filecoin",name:"Filecoin",symbol:"FIL",cgId:"filecoin",category:h.CRYPTO_ALTS},{id:"algorand",name:"Algorand",symbol:"ALGO",cgId:"algorand",category:h.CRYPTO_ALTS},{id:"fetch-ai",name:"Fetch.ai",symbol:"FET",cgId:"fetch-ai",category:h.CRYPTO_ALTS}];async function Xe(s="EUR"){const e=U.map(a=>a.cgId).filter(Boolean).join(","),t=`${We}/coins/markets?vs_currency=${s.toLowerCase()}&ids=${e}&order=market_cap_desc&per_page=100&page=1&sparkline=false&price_change_percentage=24h,7d,30d,1y`;try{const a=await fetch(t),n=a.ok?await a.json():[],i=U.filter(l=>l.yahooId),o=await Je(i);return U.map(l=>{if(l.cgId){const c=n.find(u=>u.id===l.cgId);if(c)return{...l,price:c.current_price,change24h:c.price_change_percentage_24h_in_currency||c.price_change_percentage_24h||0,change7d:c.price_change_percentage_7d_in_currency||0,change30d:c.price_change_percentage_30d_in_currency||0,change1y:c.price_change_percentage_1y_in_currency||0}}if(l.yahooId&&o[l.yahooId]){const c=o[l.yahooId],u=s==="USD"?1:.92;return{...l,price:c.price*u,change24h:c.change24h,change7d:c.change7d,change30d:c.change30d,change1y:c.change1y}}return{...l,price:null,change24h:null,change7d:null,change30d:null,change1y:null}})}catch(a){return console.error("Market fetch failed",a),U.map(n=>({...n,price:null,change24h:null,change7d:null,change30d:null,change1y:null}))}}async function Je(s){const e={};return await Promise.all(s.map(async t=>{try{const a=`https://query1.finance.yahoo.com/v8/finance/chart/${t.yahooId}?interval=1d&range=1y`,n=`https://api.allorigins.win/get?url=${encodeURIComponent(a)}`,o=await(await fetch(n)).json(),l=JSON.parse(o.contents);if(!l.chart||!l.chart.result||!l.chart.result[0])throw new Error("Invalid data");const c=l.chart.result[0],u=c.meta,d=c.indicators.quote[0].close.filter(Ie=>Ie!==null);if(d.length===0)throw new Error("No valid price data");const v=u.regularMarketPrice||d[d.length-1],b=u.chartPreviousClose||(d.length>1?d[d.length-2]:v),k=(v-b)/b*100,$=Math.max(0,d.length-6),P=d[$],T=(v-P)/P*100,_=Math.max(0,d.length-22),N=d[_],F=(v-N)/N*100,j=d[0],G=(v-j)/j*100;e[t.yahooId]={price:v,change24h:isNaN(k)?0:k,change7d:isNaN(T)?0:T,change30d:isNaN(F)?0:F,change1y:isNaN(G)?0:G}}catch(a){console.warn(`Failed to fetch ${t.symbol} from Yahoo`,a),e[t.yahooId]=null}})),e}let Z=[],Q=!0,f={key:"price",direction:"desc"};function Ze(){const s=p.getState(),e=s.currency||"EUR",t=s.currencySymbol||"€";if(Q&&Z.length===0)return fe(),`
        ...
        `;const a=Object.values(h);return`
        <div class="market-page stagger-children" style="padding-bottom: 80px;">
            ...
            ${a.map(n=>{const i=Z.filter(o=>o.category===n);return i.length===0?"":Qe(n,i,t)}).join("")}
        </div>
    `}function Qe(s,e,t){const a=[...e].sort((n,i)=>{let o=n[f.key],l=i[f.key];return typeof o=="string"&&(o=o.toLowerCase()),typeof l=="string"&&(l=l.toLowerCase()),o<l?f.direction==="asc"?-1:1:o>l?f.direction==="asc"?1:-1:0});return`
                        ...
                        <tbody>
                            ${a.map(n=>`
                                <tr>
                                    ...
                                    <td class="${n.change24h>=0?"text-positive":"text-negative"}" style="font-variant-numeric: tabular-nums;">${n.change24h!==null?Y(n.change24h):"-"}</td>
                                    <td class="${n.change7d>=0?"text-positive":"text-negative"}" style="font-variant-numeric: tabular-nums;">${n.change7d!==null?Y(n.change7d):"-"}</td>
                                    <td class="${n.change30d>=0?"text-positive":"text-negative"}" style="font-variant-numeric: tabular-nums;">${n.change30d!==null?Y(n.change30d):"-"}</td>
                                    <td class="${n.change1y>=0?"text-positive":"text-negative"}" style="font-variant-numeric: tabular-nums;">${n.change1y!==null?Y(n.change1y):"-"}</td>
                                </tr>
                            `).join("")}
                        </tbody>
                    </table>
    ...
    <div class="health-page stagger-children" style="padding-bottom: 80px;">
      ...
      <div class="card health-goal-card">
        ...
        <div class="goal-stats">
            ...
            <div class="goal-stat">
                ...
                <span class="goal-value">${nt(e)} kg</span>
            </div>
        </div>
      </div>
      ...
      <div class="routines-list">
        ...
        <button class="btn btn-secondary add-routine-btn" id="add-routine-btn">
            ${m("plus")} Agregar Nueva Rutina
        </button>
      </div>
    </div>
  `}function at(s){const e=s.weightLogs;return e.length<2?'<div class="empty-chart-msg">Registra al menos 2 pesos para ver el gráfico.</div>':`
        <div class="simple-chart-placeholder">
            <svg viewBox="0 0 100 40" class="burndown-line" preserveAspectRatio="none">
                ...
                <!-- Progress Line -->
                <path d="${st(e,s.weightGoal)}" fill="none" stroke="var(--accent-primary)" stroke-width="2" vector-effect="non-scaling-stroke" />
            </svg>
        </div>
    `}function st(s,e){if(s.length<2)return"";const t=s.map(r=>r.weight),a=Math.min(...t,e)-1,i=Math.max(...t,e)+1-a,o=s[0].date,c=s[s.length-1].date-o||1;return`M${s.map((r,d)=>{const v=d===0?0:(r.date-o)/c*100,b=40-(r.weight-a)/i*40;return`${v},${b}`}).join(" L")}`}function nt(s){if(s.weightLogs.length===0)return"--";const t=s.weightLogs[s.weightLogs.length-1].weight-s.weightGoal;return t>0?t.toFixed(1):"¡Logrado!"}function it(s){const e=new Date().setHours(0,0,0,0);return s.calorieLogs.filter(t=>new Date(t.date).setHours(0,0,0,0)===e).reduce((t,a)=>t+a.calories,0)}function ot(){var s,e,t,a;(s=document.getElementById("log-weight-btn"))==null||s.addEventListener("click",async()=>{const n=await g.prompt("Registrar Peso","Ingresa tu peso actual en kg:","Ej: 75.5","number");n&&!isNaN(n)&&(p.addWeightLog(n),g.toast("Peso registrado"))}),(e=document.getElementById("log-fat-btn"))==null||e.addEventListener("click",async()=>{const n=await g.prompt("Grasa Corporal","Ingresa tu % de grasa:","Ej: 18.2","number");n&&!isNaN(n)&&(p.addFatLog(n),g.toast("Grasa registrada"))}),(t=document.getElementById("ai-scan-photo"))==null||t.addEventListener("click",()=>{const n=document.createElement("input");n.type="file",n.accept="image/*",n.onchange=async o=>{var c;const l=o.target.files[0];if(l){if(!le.hasKey()){if(await g.confirm("IA no configurada","Para usar IA real debes añadir tu Gemini API Key en Ajustes. ¿Quieres ir a Ajustes?","Configurar IA","Usar Simulación")){(c=document.querySelector('[data-nav="settings"]'))==null||c.click();return}g.toast("Ejecutando Simulación...","info"),i();return}try{g.toast("Analizando con Gemini AI...","info");const u=await le.analyzeFood(l);await g.confirm("IA Detectada",`La IA identificó: "${u.name}" con unas ${u.calories} kcal. ¿Deseas registrarlo?`,"Registrar","Cancelar")&&(p.addCalorieLog(u.calories,`${u.name} (Gemini AI)`),g.toast("Calorías registradas con IA real"))}catch(u){console.error(u),g.alert("Error IA",u.message||"No se pudo analizar la imagen.")}}};function i(){setTimeout(async()=>{const o=[{name:"Ensalada César",cals:350},{name:"Poke Bowl de Atún",cals:520},{name:"Hamburguesa con Queso",cals:780},{name:"Pasta Carbonara",cals:650},{name:"Tostada de Aguacate",cals:280},{name:"Salmón a la Plancha",cals:410},{name:"Pizza Margherita",cals:850}],l=o[Math.floor(Math.random()*o.length)];await g.confirm("IA Simulada",`La visión (simulada) detectó "${l.name}" con ${l.cals} kcal. ¿Registrar?`)&&(p.addCalorieLog(l.cals,`${l.name} (Simulación)`),g.toast("Calorías registradas"))},1500)}n.click()}),(a=document.getElementById("add-routine-btn"))==null||a.addEventListener("click",async()=>{const n=await g.prompt("Nueva Rutina","Nombre de la rutina (ej: Pierna):","Nombre");n&&(p.saveRoutine({name:n,exercises:[]}),g.toast("Rutina creada"))})}function rt(){const s=p.getState(),{goals:e}=s;return`
      ...
      ${[{id:"day",label:"Hoy",icon:"zap"},{id:"week",label:"Esta Semana",icon:"calendar"},{id:"year",label:"Objetivos 2026",icon:"target"},{id:"long",label:"Largo Plazo",icon:"trendingUp"}].map(a=>`
        <div class="section-divider">
            <span class="section-title">
                ${m(a.icon,"section-title-icon")} ${a.label}
            </span>
  ...
  `}function lt(s,e){const t=s.filter(a=>a.timeframe===e);return t.length===0?'<div class="empty-goals">No hay metas para este periodo.</div>':t.map(a=>`
        <div class="card goal-item ${a.completed?"completed":""}" data-id="${a.id}">
            ...
            <div class="goal-content">
                <div class="goal-top">
                    ...
                    <button class="goal-delete" data-id="${a.id}">${m("trash")}</button>
                </div>
    ...
    <div class="calendar-page stagger-children" style="padding-bottom: 80px;">
      ...
      <div class="card calendar-view-card">
          <div class="calendar-mini-header">
              <button class="icon-btn-navigation">${m("chevronLeft")}</button>
              <span class="current-month">${pt()}</span>
              <button class="icon-btn-navigation">${m("chevronRight")}</button>
          </div>
            ...
            <div class="card event-card">
                ...
                <div class="event-main-col">
                    ...
                    <div class="event-details-row">
                        ...
                        ${t.repeat!=="none"?`<span class="event-repeat-tag">${gt(t.repeat)}</span>`:""}
                    </div>
                </div>
  ...
  `}function bt(s,e){if(s.length===0)return`
            <div class="empty-state">
        ...
        <div class="asset-list">
            ${s.map(a=>{const n=p.convertValue(a.amount,a.currency||"EUR"),i=ft(a.category);return`
    ...
    <div class="modal">
      ...
      <div class="type-selector category-selector">
        ${Object.entries(te).map(([e,t])=>`
          <div class="type-option ${e===I?"active":""}" data-category="${e}">
  ...
  `}function we(){const s=te[I],e=I==="investmentAsset"||I==="passiveAsset";if(e){const a=U.map(n=>({value:n.symbol,label:`${n.name} (${n.symbol})`}));[...ce,...a]}let t="";return I==="passiveAsset"?t=`
      <div class="form-row">
        <div class="form-group">
    ...
    `:I==="activeIncome"||I==="livingExpense"?t=`
      <div class="form-group">
        ...
        <div class="form-group" style="flex: 1.5;">
            ...
            <select class="form-input form-select" id="input-currency">
                <optgroup label="Divisas">
                    ${ce.map(a=>`<option value="${a.value}">${a.label}</option>`).join("")}
                </optgroup>
                ${e?`
                <optgroup label="Mercados Reales (Auto-Price)">
                    ${U.map(a=>`<option value="${a.symbol}">${a.name} (${a.symbol})</option>`).join("")}
                </optgroup>
                `:""}
            </select>
        </div>
  ...
  `}function kt(){const s=document.getElementById("add-modal"),e=document.getElementById("modal-close");s.addEventListener("click",a=>{a.target===s&&ee()}),e.addEventListener("click",ee);const t=s.querySelectorAll(".category-selector .type-option");t.forEach(a=>{a.addEventListener("click",()=>{I=a.dataset.category,t.forEach(n=>n.classList.remove("active")),a.classList.add("active"),document.getElementById("form-container").innerHTML=we(),ue()})}),ue()}function ue(){const s=document.getElementById("btn-save");s&&s.addEventListener("click",It);const e=document.getElementById("input-currency"),t=document.getElementById("input-name");e&&t&&e.addEventListener("change",()=>{if(!t.value){const a=e.options[e.selectedIndex].text;t.value=a.split(" (")[0]}})}function It(){var d,v,b,k,$,P,T,_,N,F,j,G;const s=(v=(d=document.getElementById("input-name"))==null?void 0:d.value)==null?void 0:v.trim(),e=(b=document.getElementById("input-type"))==null?void 0:b.value,t=(k=document.getElementById("input-currency"))==null?void 0:k.value,a=(P=($=document.getElementById("input-details"))==null?void 0:$.value)==null?void 0:P.trim(),n=parseFloat((T=document.getElementById("input-value"))==null?void 0:T.value)||0,i=parseFloat((_=document.getElementById("input-amount"))==null?void 0:_.value)||0,o=parseFloat((N=document.getElementById("input-monthly"))==null?void 0:N.value)||0,l=(F=document.getElementById("input-date"))==null?void 0:F.value,c=(j=document.getElementById("input-time"))==null?void 0:j.value,u=(G=document.getElementById("input-repeat"))==null?void 0:G.value;if(!s){g.alert("Campo Obligatorio","Por favor ingresa un nombre para el elemento.");return}const r={name:s,type:e,currency:t,details:a};switch(I){case"passiveAsset":p.addPassiveAsset({...r,value:n,monthlyIncome:o});break;case"activeIncome":p.addActiveIncome({...r,amount:i});break;case"livingExpense":p.addLivingExpense({...r,amount:i});break;case"investmentAsset":p.addInvestmentAsset({...r,value:n});break;case"liability":p.addLiability({...r,amount:i,monthlyPayment:o});break;case"event":p.addEvent({title:s,date:l,time:c,repeat:u,category:e});break}ee()}function ee(){const s=document.getElementById("add-modal");s&&(s.classList.remove("active"),setTimeout(()=>s.remove(),300))}function St(){const s=w.isSetup(),e=w.isBioEnabled();return`
    ...
    `}function xt(s){const e=document.getElementById("auth-submit-btn"),t=document.getElementById("auth-bio-btn"),a=document.getElementById("auth-password"),n=async()=>{const i=a.value,o=document.getElementById("auth-confirm"),l=w.isSetup();try{let c;if(l)c=await w.unlock(i);else{if(!i||i.length<4)throw new Error("Contraseña demasiado corta");if(i!==o.value)throw new Error("Las contraseñas no coinciden");c=await w.setup(i)}await p.loadEncrypted(c),s()}catch(c){g.alert("Error",c.message)}};e==null||e.addEventListener("click",n),a==null||a.addEventListener("keypress",i=>{i.key==="Enter"&&n()}),t==null||t.addEventListener("click",async()=>{try{const i=await w.unlockWithBiometrics();await p.loadEncrypted(i),s()}catch(i){g.alert("Identificación",i.message)}}),w.isBioEnabled()&&setTimeout(async()=>{try{const i=await w.unlockWithBiometrics();await p.loadEncrypted(i),s()}catch{console.log("Auto-bio failed or cancelled")}},500)}const At="modulepreload",$t=function(s){return"/life-dashboard/"+s},pe={},Ct=function(e,t,a){let n=Promise.resolve();if(t&&t.length>0){document.getElementsByTagName("link");const o=document.querySelector("meta[property=csp-nonce]"),l=(o==null?void 0:o.nonce)||(o==null?void 0:o.getAttribute("nonce"));n=Promise.allSettled(t.map(c=>{if(c=$t(c),c in pe)return;pe[c]=!0;const u=c.endsWith(".css"),r=u?'[rel="stylesheet"]':"";if(document.querySelector(`link[href="${c}"]${r}`))return;const d=document.createElement("link");if(d.rel=u?"stylesheet":At,u||(d.as="script"),d.crossOrigin="",d.href=c,l&&d.setAttribute("nonce",l),document.head.appendChild(d),u)return new Promise((v,b)=>{d.addEventListener("load",v),d.addEventListener("error",()=>b(new Error(`Unable to preload CSS for ${c}`)))})}))}function i(o){const l=new Event("vite:preloadError",{cancelable:!0});if(l.payload=o,window.dispatchEvent(l),!l.defaultPrevented)throw o}return n.then(o=>{for(const l of o||[])l.status==="rejected"&&i(l.reason);return e().catch(i)})};function Lt(){const s=w.isBioEnabled(),e=x.hasToken();return`
    ...
    `}function Tt(){var a,n,i,o,l,c;(a=document.getElementById("toggle-bio"))==null||a.addEventListener("change",async u=>{if(u.target.checked){const d=await g.prompt("Activar Biometría","Introduce tu contraseña maestra para confirmar:","Tu contraseña","password");if(d)try{await w.registerBiometrics(d),g.toast("Biometría activada correctamente")}catch(v){await g.alert("Error",v.message),u.target.checked=!1}else u.target.checked=!1}else localStorage.setItem("db_bio_enabled","false"),g.toast("Biometría desactivada","info")});const s=document.getElementById("btn-install-pwa");s&&setTimeout(()=>{if(window.deferredPrompt){const u=document.getElementById("install-pwa-card");u&&(u.style.display="block"),s.addEventListener("click",async()=>{if(!window.deferredPrompt)return;window.deferredPrompt.prompt();const{outcome:r}=await window.deferredPrompt.userChoice;if(r==="accepted"){g.toast("Instalando aplicación...");const d=document.getElementById("install-pwa-card");d&&(d.style.display="none")}window.deferredPrompt=null})}},1e3),(n=document.getElementById("sync-drive-btn"))==null||n.addEventListener("click",async()=>{const u=document.getElementById("sync-drive-btn"),r=x.hasToken(),d=u.innerHTML;try{if(u.innerHTML='<div class="loading-spinner-sm"></div>',u.style.pointerEvents="none",r){const v=w.getVaultKey();await x.pushData(p.getState(),v),g.toast("Bóveda actualizada en Drive")}else{await x.authenticate();const v=w.getVaultKey(),b=await x.pullData(v).catch(()=>null);if(b&&await g.confirm("Respaldo Encontrado","Hemos detectado una bóveda existente en Google Drive. ¿Qué deseas hacer con tus datos?","Recuperar de la Nube","Sobreescribir Nube")){p.setState(b),await p.saveState(),g.toast("Datos recuperados correctamente"),setTimeout(()=>window.location.reload(),800);return}await x.pushData(p.getState(),v),g.toast("Google Drive conectado y sincronizado")}typeof window.reRender=="function"&&window.reRender()}catch(v){if(console.error(v),v.message&&(v.message.includes("Contraseña incorrecta")||v.message.includes("corruptos"))){if(await g.confirm("Error de Decifrado","La contraseña actual no coincide con la del backup en la nube. ¿Deseas borrar los datos en Drive para empezar de cero?","Borrar Datos Nube","Cancelar"))try{await x.deleteBackup(),g.toast("Datos de Drive borrados");const k=w.getVaultKey();await x.pushData(p.getState(),k),g.toast("Google Drive sincronizado (Nueva Bóveda)")}catch{g.alert("Error","No se pudieron borrar los datos de Drive.")}}else g.alert("Error",v.message||"Error al conectar con Google")}finally{u.innerHTML=d,u.style.pointerEvents="auto"}});const e=document.getElementById("import-backup-btn"),t=document.getElementById("import-backup-input");e==null||e.addEventListener("click",()=>{t==null||t.click()}),t==null||t.addEventListener("change",async u=>{var v;const r=(v=u.target.files)==null?void 0:v[0];if(!r)return;if(!await g.confirm("¿Importar Backup?","Esto sobreescribirá todos tus datos locales con los del archivo. ¿Deseas continuar?")){t.value="";return}try{const b=await r.text(),k=JSON.parse(b),$=w.getVaultKey(),{SecurityService:P}=await Ct(async()=>{const{SecurityService:_}=await Promise.resolve().then(()=>Le);return{SecurityService:_}},void 0),T=await P.decrypt(k,$);if(T)p.setState(T),await p.saveState(),g.toast("Backup importado correctamente"),setTimeout(()=>window.location.reload(),1e3);else throw new Error("No se pudo descifrar el archivo")}catch(b){console.error("Import error:",b),g.alert("Error de Importación","El archivo no es válido o la contraseña no coincide con la usada para el backup.")}finally{t.value=""}}),(i=document.getElementById("btn-logout"))==null||i.addEventListener("click",async()=>{await g.confirm("¿Cerrar sesión?","El acceso quedará bloqueado hasta que introduzcas tu clave.")&&(w.logout(),window.location.reload())}),(o=document.getElementById("btn-force-update"))==null||o.addEventListener("click",async()=>{if(await g.confirm("¿Forzar Actualización?","Esto recargará la página y limpiará la caché para obtener la última versión.")){if(window.caches)try{const r=await caches.keys();for(let d of r)await caches.delete(d)}catch(r){console.error("Error clearing cache",r)}window.location.reload(!0)}}),(l=document.getElementById("btn-save-gemini"))==null||l.addEventListener("click",()=>{var r;const u=(r=document.getElementById("gemini-api-key"))==null?void 0:r.value;u!==void 0&&(localStorage.setItem("db_gemini_api_key",u.trim()),g.toast("API Key de Gemini guardada"))}),(c=document.getElementById("btn-factory-reset"))==null||c.addEventListener("click",async()=>{if(await g.hardConfirm("Borrar todos los datos","Esta acción eliminará permanentemente todos tus activos, ingresos, agenda y configuraciones de este dispositivo.","BORRAR")){if(localStorage.clear(),sessionStorage.clear(),window.indexedDB.databases&&(await window.indexedDB.databases()).forEach(d=>window.indexedDB.deleteDatabase(d.name)),navigator.serviceWorker){const r=await navigator.serviceWorker.getRegistrations();for(let d of r)d.unregister()}g.toast("Aplicación reseteada","info"),setTimeout(()=>{window.location.href=window.location.origin+"?reset="+Date.now()},1e3)}})}let B=localStorage.getItem("app_current_page")||"finance",E=localStorage.getItem("app_current_sub_page")||null;E==="null"&&(E=null);async function me(){const s=w.getVaultKey();s?(await p.loadEncrypted(s),Ee()):Bt()}function Bt(){const s=document.getElementById("app");s.innerHTML=St(),xt(()=>{Ee()})}function Ee(){const s=document.getElementById("app");s.innerHTML=`
        <main id="main-content"></main>
        <nav id="bottom-nav"></nav>
    `,ke(),p.subscribe(()=>{A()}),window.reRender=()=>A(),Pt()}function Pt(){var a,n;const s=localStorage.getItem("pwa_install_dismissed");if(s&&(Date.now()-parseInt(s))/864e5<7||window.matchMedia("(display-mode: standalone)").matches)return;const e=document.createElement("div");e.className="pwa-install-banner",e.id="pwa-install-banner",e.innerHTML=`
    ...
    `,document.body.appendChild(e);const t=()=>{window.deferredPrompt&&setTimeout(()=>{e.classList.add("visible")},2e3)};t(),window.addEventListener("beforeinstallprompt",t),(a=document.getElementById("pwa-banner-install"))==null||a.addEventListener("click",async()=>{if(!window.deferredPrompt)return;window.deferredPrompt.prompt();const{outcome:i}=await window.deferredPrompt.userChoice;i==="accepted"&&(e.classList.remove("visible"),setTimeout(()=>e.remove(),500)),window.deferredPrompt=null}),(n=document.getElementById("pwa-banner-close"))==null||n.addEventListener("click",()=>{e.classList.remove("visible"),localStorage.setItem("pwa_install_dismissed",Date.now().toString()),setTimeout(()=>e.remove(),500)})}function ke(){const s=document.getElementById("bottom-nav");s.innerHTML=ne(B),ie(e=>{B=e,E=null,localStorage.setItem("app_current_page",B),localStorage.setItem("app_current_sub_page",E),K(),A(),s.innerHTML=ne(B),ie(t=>{B=t,E=null,localStorage.setItem("app_current_page",B),localStorage.setItem("app_current_sub_page",E),K(),A(),ke()})}),Mt(),A()}function A(){const s=document.getElementById("main-content");if(E==="compound"){s.innerHTML=ze(),Ye(()=>{E=null,qe(),K(),A()});return}if(E==="expenses"){s.innerHTML=yt(),wt(()=>{E=null,K(),A()});return}if(E==="market"){s.innerHTML=Ze(),et(()=>{E=null,K(),A()});return}switch(B){case"finance":s.innerHTML=oe(),re(),ve();break;case"goals":s.innerHTML=rt(),ct();break;case"calendar":s.innerHTML=dt(),ht();break;case"health":s.innerHTML=tt(),ot();break;case"settings":s.innerHTML=Lt(),Tt(),q();break;default:s.innerHTML=oe(),re(),ve()}}function ve(){const s=document.getElementById("open-compound");s&&s.addEventListener("click",()=>{E="compound",localStorage.setItem("app_current_sub_page",E),q(),A()});const e=document.getElementById("open-markets");e&&e.addEventListener("click",()=>{E="market",localStorage.setItem("app_current_sub_page",E),q(),A()});const t=document.getElementById("open-expenses");t&&t.addEventListener("click",()=>{E="expenses",localStorage.setItem("app_current_sub_page",E),q(),A()})}function Mt(){const s=document.querySelector(".fab");s&&s.remove();const e=document.createElement("button");e.className="fab",e.id="main-fab",e.innerHTML=m("plus","fab-icon"),e.setAttribute("aria-label","Agregar"),e.addEventListener("click",()=>{B==="calendar"?de("event"):de()}),document.body.appendChild(e)}function q(){const s=document.getElementById("main-fab");s&&(s.style.display="none")}function K(){const s=document.getElementById("main-fab");s&&(s.style.display="flex")}document.readyState==="loading"?document.addEventListener("DOMContentLoaded",me):me();window.addEventListener("beforeinstallprompt",s=>{s.preventDefault(),window.deferredPrompt=s,console.log("PWA Install Prompt ready");const e=document.getElementById("install-pwa-card");e&&(e.style.display="block")});
